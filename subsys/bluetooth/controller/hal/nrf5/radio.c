...
void radio_phy_set(u8_t phy, u8_t flags)
{
    ...
    case BIT(2):
        ...
        /* Workaround: nRF52840 Engineering A Errata ID 164 */
        *(volatile u32_t*)0x4000173c |= 0x80000000;
        *(volatile u32_t*)0x4000173c = ((*(volatile u32_t*)0x4000173c) & 0xFFFFFF00) | 0x5C;
        break;
#endif /* CONFIG_SOC_NRF52840 */
#endif /* CONFIG_SOC_SERIES_NRF52X */
    }
...
void radio_whiten_iv_set(u32_t iv)
{
    ...
    NRF_RADIO->PCNF1 |= ((1UL) << RADIO_PCNF1_WHITEEN_Pos) & RADIO_PCNF1_WHITEEN_Msk;
}

void radio_aa_set(u8_t* aa)
{
    NRF_RADIO->TXADDRESS = (((0UL) << RADIO_TXADDRESS_TXADDRESS_Pos) & RADIO_TXADDRESS_TXADDRESS_Msk);
...
void radio_pkt_configure(u8_t bits_len, u8_t max_len, u8_t flags)
{
    ...
    case BIT(0):
    default:
        extra |= (RADIO_PCNF0_PLEN_8bit << RADIO_PCNF0_PLEN_Pos) & RADIO_PCNF0_PLEN_Msk;
        break;

    case BIT(1):
        extra |= (RADIO_PCNF0_PLEN_16bit << RADIO_PCNF0_PLEN_Pos) & RADIO_PCNF0_PLEN_Msk;
        break;

#if defined(CONFIG_SOC_NRF52840)
    case BIT(2):
        extra |= (RADIO_PCNF0_PLEN_LongRange << RADIO_PCNF0_PLEN_Pos) & RADIO_PCNF0_PLEN_Msk;
        extra |= (2UL << RADIO_PCNF0_CILEN_Pos) & RADIO_PCNF0_CILEN_Msk;
        extra |= (3UL << RADIO_PCNF0_TERMLEN_Pos) & RADIO_PCNF0_TERMLEN_Msk;
        break;
#endif /* CONFIG_SOC_NRF52840 */
    }
    ...
    NRF_RADIO->PCNF0 = (((1UL) << RADIO_PCNF0_S0LEN_Pos) & RADIO_PCNF0_S0LEN_Msk) | ((((u32_t)bits_len) << RADIO_PCNF0_LFLEN_Pos) & RADIO_PCNF0_LFLEN_Msk) | ((((u32_t)8 - bits_len) << RADIO_PCNF0_S1LEN_Pos) & RADIO_PCNF0_S1LEN_Msk) | extra;

    NRF_RADIO->PCNF1 &= ~(RADIO_PCNF1_MAXLEN_Msk | RADIO_PCNF1_STATLEN_Msk | RADIO_PCNF1_BALEN_Msk | RADIO_PCNF1_ENDIAN_Msk);
    NRF_RADIO->PCNF1 |= ((((u32_t)max_len) << RADIO_PCNF1_MAXLEN_Pos) & RADIO_PCNF1_MAXLEN_Msk) | (((0UL) << RADIO_PCNF1_STATLEN_Pos) & RADIO_PCNF1_STATLEN_Msk) | (((3UL) << RADIO_PCNF1_BALEN_Pos) & RADIO_PCNF1_BALEN_Msk) | (((RADIO_PCNF1_ENDIAN_Little) << RADIO_PCNF1_ENDIAN_Pos) & RADIO_PCNF1_ENDIAN_Msk);
}
...
static u8_t MALIGN(4) _pkt_scratch[((RADIO_PDU_LEN_MAX + 3) > PDU_AC_SIZE_MAX) ? (RADIO_PDU_LEN_MAX + 3) : PDU_AC_SIZE_MAX];
    ...
    u8_t flags_next)
{
    ...
    if (dir) {
        delay = radio_tx_ready_delay_get(phy_next, flags_next) + radio_rx_chain_delay_get(phy_curr, 1);

        NRF_PPI->CH[ppi].TEP = (u32_t) & (NRF_RADIO->TASKS_TXEN);
    } else {
        delay = radio_rx_ready_delay_get(phy_next) - radio_tx_chain_delay_get(phy_curr, flags_curr) + 4; /* 4us as +/- active jitter */
    ...
    u8_t* bdaddr)
{
    ...
    for (index = 0; index < 8; index++) {
        NRF_RADIO->DAB[index] = ((u32_t)bdaddr[3] << 24) | ((u32_t)bdaddr[2] << 16) | ((u32_t)bdaddr[1] << 8) | bdaddr[0];
...
void radio_tmr_status_reset(void)
{
    ...
    NRF_PPI->CHENCLR = (PPI_CHEN_CH0_Msk | PPI_CHEN_CH1_Msk | PPI_CHEN_CH2_Msk | PPI_CHEN_CH3_Msk | PPI_CHEN_CH4_Msk | PPI_CHEN_CH5_Msk | PPI_CHEN_CH6_Msk | PPI_CHEN_CH15_Msk);
}
...
u32_t radio_tmr_start(u8_t trx, u32_t ticks_start, u32_t remainder)
{
    ...
    NRF_PPI->CH[0].TEP = (trx) ? (u32_t) & (NRF_RADIO->TASKS_TXEN) : (u32_t) & (NRF_RADIO->TASKS_RXEN);
...
{
    NRF_GPIOTE->CONFIG[CONFIG_BT_CTLR_PA_LNA_GPIOTE_CHAN] = (GPIOTE_CONFIG_MODE_Task << GPIOTE_CONFIG_MODE_Pos) | (CONFIG_BT_CTLR_GPIO_PA_PIN << GPIOTE_CONFIG_PSEL_Pos) | (GPIOTE_CONFIG_POLARITY_Toggle << GPIOTE_CONFIG_POLARITY_Pos) |
#if defined(CONFIG_BT_CTLR_GPIO_PA_POL_INV)
...
{
    NRF_GPIOTE->CONFIG[CONFIG_BT_CTLR_PA_LNA_GPIOTE_CHAN] = (GPIOTE_CONFIG_MODE_Task << GPIOTE_CONFIG_MODE_Pos) | (CONFIG_BT_CTLR_GPIO_LNA_PIN << GPIOTE_CONFIG_PSEL_Pos) | (GPIOTE_CONFIG_POLARITY_Toggle << GPIOTE_CONFIG_POLARITY_Pos) |
...
void* radio_ccm_rx_pkt_set(struct ccm* ccm, u8_t phy, void* pkt)
{
    ...
    mode = (CCM_MODE_MODE_Decryption << CCM_MODE_MODE_Pos) & CCM_MODE_MODE_Msk;
#if defined(CONFIG_SOC_SERIES_NRF52X)
    /* Enable CCM support for 8-bit length field PDUs. */
    mode |= (CCM_MODE_LENGTH_Extended << CCM_MODE_LENGTH_Pos) & CCM_MODE_LENGTH_Msk;

    /* Select CCM data rate based on current PHY in use. */
<<<<<<< HEAD
    /* TODO: add cases for nRF52840's coded PHY */
    switch ((NRF_RADIO->MODE & RADIO_MODE_MODE_Msk) >> RADIO_MODE_MODE_Pos) {
    default:
...
=======
    ...
    case BIT(0):
>>>>>>> master
        mode |= (CCM_MODE_DATARATE_1Mbit << CCM_MODE_DATARATE_Pos) & CCM_MODE_DATARATE_Msk;
        break;
...
=======
    case BIT(1):
>>>>>>> master
        mode |= (CCM_MODE_DATARATE_2Mbit << CCM_MODE_DATARATE_Pos) & CCM_MODE_DATARATE_Msk;
        break;
<<<<<<< HEAD
=======

        #if defined(CONFIG_SOC_NRF52840) case BIT(2)
            : mode |= (CCM_MODE_DATARATE_125Kbps << CCM_MODE_DATARATE_Pos) & CCM_MODE_DATARATE_Msk;

        NRF_CCM->RATEOVERRIDE = (CCM_RATEOVERRIDE_RATEOVERRIDE_500Kbps << CCM_RATEOVERRIDE_RATEOVERRIDE_Pos) & CCM_RATEOVERRIDE_RATEOVERRIDE_Msk;
...
void* radio_ccm_tx_pkt_set(struct ccm* ccm, void* pkt)
{
    ...
    NRF_CCM->ENABLE = CCM_ENABLE_ENABLE_Disabled;
    NRF_CCM->ENABLE = CCM_ENABLE_ENABLE_Enabled;
    mode = (CCM_MODE_MODE_Encryption << CCM_MODE_MODE_Pos) & CCM_MODE_MODE_Msk;
#if defined(CONFIG_SOC_SERIES_NRF52X)
    /* Enable CCM support for 8-bit length field PDUs. */
    mode |= (CCM_MODE_LENGTH_Extended << CCM_MODE_LENGTH_Pos) & CCM_MODE_LENGTH_Msk;
...
{
    NRF_AAR->ENABLE = (AAR_ENABLE_ENABLE_Enabled << AAR_ENABLE_ENABLE_Pos) & AAR_ENABLE_ENABLE_Msk;
    NRF_AAR->NIRK = nirk;
...
void radio_ar_status_reset(void)
{
    ...
    NRF_AAR->ENABLE = (AAR_ENABLE_ENABLE_Disabled << AAR_ENABLE_ENABLE_Pos) & AAR_ENABLE_ENABLE_Msk;
}

u32_t radio_ar_has_match(void)
{
    return (radio_bc_has_match() && NRF_AAR->EVENTS_END && NRF_AAR->EVENTS_RESOLVED && !NRF_AAR->EVENTS_NOTRESOLVED);
}
