...
#define RADIO_PKT_TIME(octets, phy) \
    (((phy)&BIT(2)) ? (80 + 256 + 16 + 24 + ((((2 + (octets) + 4) * 8) + 24 + 3) * 8)) : (((octets) + 14) * 8 / BIT(((phy)&0x03) >> 1)))
    ...
    u16_t mem_size)
{
    ...
    if ((PDU_AC_SIZE_MAX + PDU_AC_SIZE_EXTRA) < (offsetof(struct pdu_data, payload) + _radio.packet_data_octets_max)) {
        _radio.packet_rx_data_pool_size = (MROUND(offsetof(struct radio_pdu_node_rx, pdu_data) + offsetof(struct pdu_data, payload) + _radio.packet_data_octets_max) * rx_count_max);
    } else {
        _radio.packet_rx_data_pool_size = (MROUND(offsetof(struct radio_pdu_node_rx, pdu_data) + (PDU_AC_SIZE_MAX + PDU_AC_SIZE_EXTRA)) * rx_count_max);
    }
    _radio.packet_rx_data_size = PACKET_RX_DATA_SIZE_MIN;
    _radio.packet_rx_data_count = (_radio.packet_rx_data_pool_size / _radio.packet_rx_data_size);
    ...
    mem_radio += (sizeof(void*) * 2 * (_radio.packet_rx_count + _radio.connection_count));
    ...
    _radio.packet_tx_data_size = MROUND(offsetof(struct radio_pdu_node_tx, pdu_data) + offsetof(struct pdu_data, payload) + packet_tx_data_size);
...
static inline void isr_radio_state_tx(void)
{
...
#if defined(CONFIG_BT_CTLR_GPIO_LNA_PIN)
        radio_gpio_pa_lna_enable(radio_tmr_end_get() + RADIO_TIFS - 4 - radio_tx_chain_delay_get(0, 0) - CONFIG_BT_CTLR_GPIO_LNA_OFFSET);
#endif /* CONFIG_BT_CTLR_GPIO_LNA_PIN */
        break;
    ...
    case ROLE_SLAVE:
        ...
        rx_packet_set(_radio.conn_curr, (struct pdu_data*)_radio.packet_rx[_radio.packet_rx_last]->pdu_data);
...
#if defined(CONFIG_BT_CTLR_GPIO_LNA_PIN)
#if defined(CONFIG_BT_CTLR_PHY)
        radio_gpio_pa_lna_enable(radio_tmr_end_get() + RADIO_TIFS - 4 - radio_tx_chain_delay_get(
                                                                            _radio.conn_curr->phy_tx,
                                                                            _radio.conn_curr->phy_flags)
...
static u32_t isr_rx_adv_sr_report(struct pdu_adv* pdu_adv_rx, u8_t rssi_ready)
{
    ...
    ((u8_t*)pdu_adv)[pdu_len] = (rssi_ready) ? (radio_rssi_get() & 0x7f) : 0x7f;
...
static inline bool isr_adv_sr_adva_check(struct pdu_adv* adv,
    struct pdu_adv* sr)
{
    return (adv->tx_addr == sr->rx_addr) && !memcmp(adv->payload.adv_ind.addr, sr->payload.scan_req.adv_addr, BDADDR_SIZE);
}
...
{
#if defined(CONFIG_BT_CTLR_PRIVACY)
    return ((((_radio.advertiser.filter_policy & 0x01) == 0) && ctrl_rl_addr_allowed(sr->tx_addr,
                                                                    sr->payload.scan_req.scan_addr,
                                                                    rl_idx))
               || (((_radio.advertiser.filter_policy & 0x01) != 0) && (devmatch_ok || ctrl_irk_whitelisted(*rl_idx))))
        && isr_adv_sr_adva_check(adv, sr);
#else
    return (((_radio.advertiser.filter_policy & 0x01) == 0) || (devmatch_ok)) && isr_adv_sr_adva_check(adv, sr);
#endif /* CONFIG_BT_CTLR_PRIVACY */
}
...
#endif /* CONFIG_BT_CTLR_PRIVACY */
    return (adv->rx_addr == ci->tx_addr) && !memcmp(adv->payload.direct_ind.tgt_addr, ci->payload.connect_ind.init_addr, BDADDR_SIZE);
}

static inline bool isr_adv_ci_adva_check(struct pdu_adv* adv,
    struct pdu_adv* ci)
{
    return (adv->tx_addr == ci->rx_addr) && (((adv->type == PDU_ADV_TYPE_DIRECT_IND) && !memcmp(adv->payload.direct_ind.adv_addr, ci->payload.connect_ind.adv_addr, BDADDR_SIZE)) || (!memcmp(adv->payload.adv_ind.addr,
    ...
    u8_t devmatch_ok, u8_t* rl_idx)
{
    ...
    if (adv->type == PDU_ADV_TYPE_DIRECT_IND) {
        ...
        return (1) &&
#endif
            isr_adv_ci_adva_check(adv, ci) && isr_adv_ci_tgta_check(adv, ci, *rl_idx);
    }

#if defined(CONFIG_BT_CTLR_PRIVACY)
    return ((((_radio.advertiser.filter_policy & 0x02) == 0) && ctrl_rl_addr_allowed(ci->tx_addr,
                                                                    ci->payload.connect_ind.init_addr,
                                                                    rl_idx))
               || (((_radio.advertiser.filter_policy & 0x02) != 0) && (devmatch_ok || ctrl_irk_whitelisted(*rl_idx))))
        && isr_adv_ci_adva_check(adv, ci);
#else
    return (((_radio.advertiser.filter_policy & 0x02) == 0) || (devmatch_ok)) && isr_adv_ci_adva_check(adv, ci);
#endif /* CONFIG_BT_CTLR_PRIVACY */
}
    ...
    u8_t rssi_ready)
{
    ...
    u8_t rl_idx = irkmatch_ok ? ctrl_rl_irk_idx(irkmatch_id) : FILTER_IDX_NONE;
#else
    ...
    if ((pdu_adv->type == PDU_ADV_TYPE_SCAN_REQ) && (pdu_adv->len == sizeof(struct pdu_adv_payload_scan_req)) && isr_adv_sr_check(_pdu_adv, pdu_adv, devmatch_ok, &rl_idx)) {

#if defined(CONFIG_BT_CTLR_SCAN_REQ_NOTIFY)
        if (!IS_ENABLED(CONFIG_BT_CTLR_ADV_EXT) || 0 /* TODO: extended adv. scan req notification enabled */) {
            u32_t err;
        ...
        radio_pkt_tx_set(&_radio.advertiser.scan_data.data[_radio.advertiser.scan_data.first][0]);

#if defined(CONFIG_BT_CTLR_GPIO_PA_PIN)
        radio_gpio_pa_setup();
        radio_gpio_pa_lna_enable(radio_tmr_end_get() + RADIO_TIFS - radio_rx_chain_delay_get(0, 0) - CONFIG_BT_CTLR_GPIO_PA_OFFSET);
        ...
        return 0;
    } else if ((pdu_adv->type == PDU_ADV_TYPE_CONNECT_IND) && (pdu_adv->len == sizeof(struct pdu_adv_payload_connect_ind)) && isr_adv_ci_check(_pdu_adv, pdu_adv, devmatch_ok, &rl_idx) && ((_radio.fc_ena == 0) || (_radio.fc_req == _radio.fc_ack)) && (_radio.advertiser.conn)) {
        ...
        conn->data_chan_count = util_ones_count_get(&conn->data_chan_map[0],
            sizeof(conn->data_chan_map));
        conn->data_chan_hop = pdu_adv->payload.connect_ind.lldata.hop;
        conn->conn_interval = pdu_adv->payload.connect_ind.lldata.interval;
        conn_interval_us = pdu_adv->payload.connect_ind.lldata.interval * 1250;
        conn->latency = pdu_adv->payload.connect_ind.lldata.latency;
        memcpy((void*)&conn->slave.force, &conn->access_addr[0],
            sizeof(conn->slave.force));
        conn->supervision_reload = RADIO_CONN_EVENTS((pdu_adv->payload.connect_ind.lldata.timeout
                                                         * 10 * 1000),
            conn_interval_us);
		 ...
		 */
        conn->appto_reload = (conn->apto_reload > (conn->latency + 6)) ? (conn->apto_reload - (conn->latency + 6)) : conn->apto_reload;
#endif /* CONFIG_BT_CTLR_LE_PING */
        ...
        radio_le_conn_cmplt = (struct radio_le_conn_cmplt*)&pdu_data->payload;
        ...
        radio_le_conn_cmplt->interval = pdu_adv->payload.connect_ind.lldata.interval;
        radio_le_conn_cmplt->latency = pdu_adv->payload.connect_ind.lldata.latency;
        radio_le_conn_cmplt->timeout = pdu_adv->payload.connect_ind.lldata.timeout;
        radio_le_conn_cmplt->mca = pdu_adv->payload.connect_ind.lldata.sca;
        ...
        if (IS_ENABLED(CONFIG_BT_CTLR_CHAN_SEL_2)) {
            ...
            radio_pdu_node_rx->hdr.handle = conn->handle;
            radio_pdu_node_rx->hdr.type = NODE_RX_TYPE_CHAN_SEL_ALGO;

            pdu_data = (struct pdu_data*)
                           radio_pdu_node_rx->pdu_data;
            le_chan_sel_algo = (struct radio_le_chan_sel_algo*)&pdu_data->payload;

            if (pdu_adv->chan_sel) {
                u16_t aa_ls = ((u16_t)conn->access_addr[1] << 8) | conn->access_addr[0];
                u16_t aa_ms = ((u16_t)conn->access_addr[3] << 8) | conn->access_addr[2];
        ...
        conn->slave.window_widening_periodic_us = (((gc_lookup_ppm[_radio.sca] + gc_lookup_ppm[conn->slave.sca]) * conn_interval_us) + (1000000 - 1)) / 1000000;
        conn->slave.window_widening_max_us = (conn_interval_us >> 1) - RADIO_TIFS;
        conn->slave.window_size_event_us = pdu_adv->payload.connect_ind.lldata.win_size * 1250;
        conn->slave.window_size_prepare_us = 0;

        rx_ready_delay = radio_rx_ready_delay_get(0);

        /* calculate slave slot */
        conn->hdr.ticks_slot = TICKER_US_TO_TICKS(RADIO_TICKER_START_PART_US + rx_ready_delay + 328 + RADIO_TIFS + 328);
        conn->hdr.ticks_active_to_start = _radio.ticks_active_to_start;
        conn->hdr.ticks_xtal_to_start = TICKER_US_TO_TICKS(RADIO_TICKER_XTAL_OFFSET_US);
        conn->hdr.ticks_preempt_to_start = TICKER_US_TO_TICKS(RADIO_TICKER_PREEMPT_PART_MIN_US);
        ticks_slot_offset = (conn->hdr.ticks_active_to_start < conn->hdr.ticks_xtal_to_start) ? conn->hdr.ticks_xtal_to_start : conn->hdr.ticks_active_to_start;
        conn_interval_us -= conn->slave.window_widening_periodic_us;

        conn_offset_us = radio_tmr_end_get();
        conn_offset_us += ((u64_t)pdu_adv->payload.connect_ind.lldata.win_offset + 1) * 1250;
        ...
        ticker_status = ticker_start(RADIO_TICKER_INSTANCE_ID_RADIO,
            ...
            (void*)__LINE__);
        LL_ASSERT((ticker_status == TICKER_STATUS_SUCCESS) || (ticker_status == TICKER_STATUS_BUSY));
...
static u32_t isr_rx_scan_report(u8_t rssi_ready, u8_t rl_idx, bool dir_report)
{
    ...
    if (0) {
        ...
        case BIT(0):
            radio_pdu_node_rx->hdr.type = NODE_RX_TYPE_EXT_1M_REPORT;
            break;

        case BIT(2):
            radio_pdu_node_rx->hdr.type = NODE_RX_TYPE_EXT_CODED_REPORT;
            break;
    ...
    ((u8_t*)pdu_adv_rx)[offsetof(struct pdu_adv, payload) + pdu_adv_rx->len + 1] = rl_idx;
#endif /* CONFIG_BT_CTLR_PRIVACY */
#if defined(CONFIG_BT_CTLR_EXT_SCAN_FP)
    /* save the directed adv report flag */
    ((u8_t*)pdu_adv_rx)[offsetof(struct pdu_adv, payload) + pdu_adv_rx->len + 2] = dir_report ? 1 : 0;
...
{
#if defined(CONFIG_BT_CTLR_PRIVACY)
    return (((_radio.scanner.filter_policy & 0x01) == 0) && (!devmatch_ok || ctrl_rl_idx_allowed(irkmatch_ok, rl_idx))) || (((_radio.scanner.filter_policy & 0x01) != 0) && (devmatch_ok || ctrl_irk_whitelisted(rl_idx)));
#else
...
static inline bool isr_scan_rsp_adva_matches(struct pdu_adv* srsp)
{
    ...
    return ((sreq->rx_addr == srsp->tx_addr) && (memcmp(&sreq->payload.scan_req.adv_addr[0],
                                                     &srsp->payload.scan_rsp.addr[0], BDADDR_SIZE)
                                                    == 0));
}
...
#endif /* CONFIG_BT_CTLR_PRIVACY */
    return ((_radio.scanner.adv_addr_type == pdu->tx_addr) && (memcmp(&_radio.scanner.adv_addr[0],
...
static inline bool isr_scan_tgta_rpa_check(struct pdu_adv* pdu,
    bool* dir_report)
{
    if (((_radio.scanner.filter_policy & 0x02) != 0) && (pdu->rx_addr != 0) && ((pdu->payload.direct_ind.tgt_addr[5] & 0xc0) == 0x40)) {
    ...
    u8_t rl_idx, bool* dir_report)
{
    ...
    return (((_radio.scanner.init_addr_type == pdu->rx_addr) && (memcmp(&_radio.scanner.init_addr[0],
...
{
    return ((((_radio.scanner.filter_policy & 0x01) != 0) || isr_scan_init_adva_check(pdu, rl_idx)) && ((pdu->type == PDU_ADV_TYPE_ADV_IND) || ((pdu->type == PDU_ADV_TYPE_DIRECT_IND) && (/* allow directed adv packets addressed to this device */
    ...
    u8_t rssi_ready)
{
    ...
    /* Initiator */
    if ((_radio.scanner.conn) && ((_radio.fc_ena == 0) || (_radio.fc_req == _radio.fc_ack)) && isr_scan_init_check(pdu_adv_rx, rl_idx) && ((radio_tmr_end_get() + 502 + (RADIO_TICKER_JITTER_US << 1)) < (TICKER_TICKS_TO_US(_radio.scanner.hdr.ticks_slot) - RADIO_TICKER_START_PART_US))) {
        struct radio_le_conn_cmplt* radio_le_conn_cmplt;
        struct radio_pdu_node_rx* radio_pdu_node_rx;
        ...
        memcpy(&pdu_adv_tx->payload.connect_ind.lldata.access_addr[0], &conn->access_addr[0], 4);
		 ...
		 * packet end.
		 */
        if (!_radio.remainder_anchor || (_radio.remainder_anchor & BIT(31))) {
        ...
        if (_radio.scanner.win_offset_us == 0) {
            ...
            while ((conn_space_us & ((u32_t)1 << 31)) || (conn_space_us < conn_offset_us)) {
                conn_space_us += conn_interval_us;
            }
            pdu_adv_tx->payload.connect_ind.lldata.win_offset = (conn_space_us - conn_offset_us) / 1250;
        ...
        pdu_adv_tx->payload.connect_ind.lldata.interval = _radio.scanner.conn_interval;
        pdu_adv_tx->payload.connect_ind.lldata.latency = _radio.scanner.conn_latency;
        pdu_adv_tx->payload.connect_ind.lldata.timeout = _radio.scanner.conn_timeout;
        ...
        pdu_adv_tx->payload.connect_ind.lldata.hop = conn->data_chan_hop;
        ...
        radio_le_conn_cmplt->mca = pdu_adv_tx->payload.connect_ind.lldata.sca;
    ...
    /* Active scanner */
    else if (((pdu_adv_rx->type == PDU_ADV_TYPE_ADV_IND) || (pdu_adv_rx->type == PDU_ADV_TYPE_SCAN_IND)) && (_radio.scanner.type != 0) && (_radio.scanner.conn == 0)) {
        struct pdu_adv* pdu_adv_tx;
#if defined(CONFIG_BT_CTLR_PRIVACY)
        bt_addr_t* lrpa;
#endif /* CONFIG_BT_CTLR_PRIVACY */
    ...
    else if (((pdu_adv_rx->type == PDU_ADV_TYPE_ADV_IND) || ((pdu_adv_rx->type == PDU_ADV_TYPE_DIRECT_IND) && (/* allow directed adv packets addressed to this device */
                                                                                                                  isr_scan_tgta_check(false, pdu_adv_rx, rl_idx,
                                                                                                                      &dir_report)))
                 || (pdu_adv_rx->type == PDU_ADV_TYPE_NONCONN_IND) || (pdu_adv_rx->type == PDU_ADV_TYPE_SCAN_IND) ||
#if defined(CONFIG_BT_CTLR_ADV_EXT)
                 ((pdu_adv_rx->type == PDU_ADV_TYPE_EXT_IND) && (_radio.scanner.phy)) ||
#endif /* CONFIG_BT_CTLR_ADV_EXT */
                 ((pdu_adv_rx->type == PDU_ADV_TYPE_SCAN_RSP) && (_radio.scanner.state != 0) && isr_scan_rsp_adva_matches(pdu_adv_rx)))
    ...
    struct radio_pdu_node_tx** node_tx)
{
    ...
    case PDU_DATA_LLCTRL_TYPE_TERMINATE_IND:
        ...
        terminate_ind_rx_enqueue(_radio.conn_curr,
            (pdu_data_tx->payload.llctrl.ctrldata.terminate_ind.error_code == 0x13) ? 0x16 : pdu_data_tx->payload.llctrl.ctrldata.terminate_ind.error_code);
    ...
    case PDU_DATA_LLCTRL_TYPE_ENC_REQ:
		 ...
		 */
        _radio.conn_curr->procedure_expire = _radio.conn_curr->procedure_reload;
        break;
    ...
    case PDU_DATA_LLCTRL_TYPE_LENGTH_REQ:
        if ((_radio.conn_curr->llcp_length.req != _radio.conn_curr->llcp_length.ack) && (_radio.conn_curr->llcp_length.state == LLCP_LENGTH_STATE_ACK_WAIT)) {
            /* pause data packet tx */
            _radio.conn_curr->pause_tx = 1;

            /* wait for response */
            _radio.conn_curr->llcp_length.state = LLCP_LENGTH_STATE_RSP_WAIT;
        }
        break;
#endif /* CONFIG_BT_CTLR_DATA_LENGTH */
    ...
    case PDU_DATA_LLCTRL_TYPE_PHY_UPD_IND:
        _radio.conn_curr->phy_tx_time = _radio.conn_curr->llcp.phy_upd_ind.tx;
        break;
#endif /* CONFIG_BT_CTLR_PHY */
...
static inline u32_t feat_get(u8_t* features)
{
    ...
    feat = ~RADIO_BLE_FEAT_BIT_MASK_VALID | features[0] | (features[1] << 8) | (features[2] << 16);
    ...
    u8_t* rx_enqueue)
{
    ...
    rej_ext_ind = (struct pdu_data_llctrl_reject_ext_ind*)&pdu_data_rx->payload.llctrl.ctrldata.reject_ext_ind;
    ...
    cp->timeout = conn->supervision_reload * conn->conn_interval * 125 / 1000;
    ...
    u8_t* rx_enqueue)
{
    ...
    if (rej_ext_ind->reject_opcode == PDU_DATA_LLCTRL_TYPE_LENGTH_REQ) {
        ...
        pdu_data_rx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_LENGTH_RSP;
    ...
    u8_t* rx_enqueue)
{
    ...
    if (rej_ext_ind->reject_opcode == PDU_DATA_LLCTRL_TYPE_PHY_REQ) {
        ...
        if (rej_ext_ind->error_code != 0x23) {
            /* Procedure complete */
            _radio.conn_curr->llcp_phy.ack = _radio.conn_curr->llcp_phy.req;

            /* Reset packet timing restrictions */
            _radio.conn_curr->phy_tx_time = _radio.conn_curr->phy_tx;
    ...
    u8_t* rx_enqueue)
{
    if (0) {
#if defined(CONFIG_BT_CTLR_PHY)
    } else if (_radio.conn_curr->llcp_phy.ack != _radio.conn_curr->llcp_phy.req) {
        isr_rx_conn_pkt_ctrl_rej_phy_upd(radio_pdu_node_rx,
            rx_enqueue);
#endif /* CONFIG_BT_CTLR_PHY */

#if defined(CONFIG_BT_CTLR_CONN_PARAM_REQ)
    } else if (_radio.conn_curr->llcp_conn_param.ack != _radio.conn_curr->llcp_conn_param.req) {
        isr_rx_conn_pkt_ctrl_rej_conn_upd(radio_pdu_node_rx,
            rx_enqueue);
#endif /* CONFIG_BT_CTLR_CONN_PARAM_REQ */

#if defined(CONFIG_BT_CTLR_DATA_LENGTH)
    } else if (_radio.conn_curr->llcp_length.ack != _radio.conn_curr->llcp_length.req) {
        ...
        rej_ext_ind = (void*)&pdu_rx->payload.llctrl.ctrldata.reject_ext_ind;
        ...
        case PDU_DATA_LLCTRL_TYPE_ENC_REQ:
            ...
            pdu_rx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_REJECT_IND;
            pdu_rx->payload.llctrl.ctrldata.reject_ind.error_code = rej_ext_ind->error_code;
            *rx_enqueue = 1;
            break;
    ...
    u8_t* rx_enqueue)
{
    ...
    if (pdu_data_rx->payload.llctrl.opcode == PDU_DATA_LLCTRL_TYPE_LENGTH_REQ) {
    ...
    if (/* Local idle, and Peer request then complete the Peer procedure
	     * with response.
	     */
        ((_radio.conn_curr->llcp_length.req == _radio.conn_curr->llcp_length.ack) && (pdu_data_rx->payload.llctrl.opcode == PDU_DATA_LLCTRL_TYPE_LENGTH_REQ)) ||
        /* or Local has active... */
        ((_radio.conn_curr->llcp_length.req != _radio.conn_curr->llcp_length.ack) &&
            /* with Local requested and Peer request then complete the
	      * Peer procedure with response.
	      */
            ((((_radio.conn_curr->llcp_length.state == LLCP_LENGTH_STATE_REQ) || (_radio.conn_curr->llcp_length.state == LLCP_LENGTH_STATE_ACK_WAIT)) && (pdu_data_rx->payload.llctrl.opcode == PDU_DATA_LLCTRL_TYPE_LENGTH_REQ)) ||
                /* with Local waiting for response, and Peer response then
	       * complete the Local procedure or Peer request then complete the
	       * Peer procedure with response.
	       */
                ((_radio.conn_curr->llcp_length.state == LLCP_LENGTH_STATE_RSP_WAIT) && ((pdu_data_rx->payload.llctrl.opcode == PDU_DATA_LLCTRL_TYPE_LENGTH_RSP) || (pdu_data_rx->payload.llctrl.opcode == PDU_DATA_LLCTRL_TYPE_LENGTH_REQ)))))) {
        struct pdu_data_llctrl_length_req_rsp* lr;

        lr = (struct pdu_data_llctrl_length_req_rsp*)&pdu_data_rx->payload.llctrl.ctrldata.length_req;
		 ...
		 * peer max_rx_time
		 */
        if (lr->max_rx_time >= RADIO_PKT_TIME(RADIO_LL_LENGTH_OCTETS_RX_MIN, 0)) {
		 ...
		 * peer max_tx_time
		 */
        if (lr->max_tx_time >= RADIO_PKT_TIME(RADIO_LL_LENGTH_OCTETS_RX_MIN, 0)) {
        ...
        if (eff_rx_octets != _radio.conn_curr->max_rx_octets) {
            ...
            if (_radio.packet_rx_data_count == free_count_rx) {
				 ...
				 */
                _radio.conn_curr->llcp_length.rx_octets = eff_rx_octets;
                _radio.conn_curr->llcp_length.tx_octets = eff_tx_octets;

#if defined(CONFIG_BT_CTLR_PHY)
                /* accept the effective tx time */
                _radio.conn_curr->max_tx_time = eff_tx_time;

                _radio.conn_curr->llcp_length.rx_time = eff_rx_time;
                _radio.conn_curr->llcp_length.tx_time = eff_tx_time;
#endif /* CONFIG_BT_CTLR_PHY */

                _radio.conn_curr->llcp_length.ack = (_radio.conn_curr->llcp_length.req - 1);
                _radio.conn_curr->llcp_length.state = LLCP_LENGTH_STATE_RESIZE;
            ...
            /* Procedure complete */
            _radio.conn_curr->llcp_length.ack = _radio.conn_curr->llcp_length.req;
            _radio.conn_curr->procedure_expire = 0;
        ...
        LL_ASSERT(pdu_data_rx->payload.llctrl.opcode == PDU_DATA_LLCTRL_TYPE_LENGTH_RSP);
    }
    ...
    u8_t* rx_enqueue)
{
    ...
    case PDU_DATA_LLCTRL_TYPE_TERMINATE_IND:
        ...
        _radio.conn_curr->llcp_terminate.reason_peer = pdu_data_rx->payload.llctrl.ctrldata.terminate_ind.error_code;
        break;
    ...
    case PDU_DATA_LLCTRL_TYPE_START_ENC_REQ:
        LL_ASSERT((_radio.conn_curr->llcp_req == _radio.conn_curr->llcp_ack) || (_radio.conn_curr->llcp_type == LLCP_ENCRYPTION));
    ...
    case PDU_DATA_LLCTRL_TYPE_CONN_PARAM_REQ:
        ...
        if (!_radio.conn_curr->role) {
            ...
            if ((conn->llcp_conn_param.req != conn->llcp_conn_param.ack) && ((conn->llcp_conn_param.state == LLCP_CPR_STATE_REQ) || (conn->llcp_conn_param.state == LLCP_CPR_STATE_RSP_WAIT) || (conn->llcp_conn_param.state == LLCP_CPR_STATE_UPD))) {
                /* Same procedure collision  */
                nack = reject_ext_ind_send(_radio.conn_curr,
                    PDU_DATA_LLCTRL_TYPE_CONN_PARAM_REQ,
                    0x23);
#if defined(CONFIG_BT_CTLR_PHY)
            } else if (((conn->llcp_req != conn->llcp_ack) && (conn->llcp_type != LLCP_ENCRYPTION)) || (conn->llcp_phy.req != conn->llcp_phy.ack)) {
#else /* !CONFIG_BT_CTLR_PHY */
            } else if ((conn->llcp_req != conn->llcp_ack) && (conn->llcp_type != LLCP_ENCRYPTION)) {
#endif /* !CONFIG_BT_CTLR_PHY */
                ...
                struct pdu_data_llctrl_conn_param_req* cpr = (void*)&pdu_data_rx->payload.llctrl.ctrldata.conn_param_req;

                /* Invalid parameters */
                if ((cpr->interval_min < 6) || (cpr->interval_max > 3200) || (cpr->interval_min > cpr->interval_max) || (cpr->latency > 499) || (cpr->timeout < 10) || (cpr->timeout > 3200) || ((cpr->timeout * 4) <= ((cpr->latency + 1) * cpr->interval_max)) || (cpr->preferred_periodicity > cpr->interval_max)) {
                ...
                conn->llcp_conn_param.preferred_periodicity = cpr->preferred_periodicity;
                conn->llcp_conn_param.reference_conn_event_count = cpr->reference_conn_event_count;
				 ...
				 */
                if ((conn->llcp_conn_param.interval != conn->conn_interval) || (conn->llcp_conn_param.latency != conn->latency) || (RADIO_CONN_EVENTS(conn->llcp_conn_param.timeout * 10000,
					 ...
					 */
                    if (conn->pause_tx) {
                        conn->llcp_conn_param.state = LLCP_CPR_STATE_APP_REQ;
                    } else {
                        *rx_enqueue = 1;
        ...
        } else if ((_radio.conn_curr->llcp_conn_param.req == _radio.conn_curr->llcp_conn_param.ack) || (_radio.conn_curr->llcp_conn_param.state == LLCP_CPR_STATE_REQ) || (_radio.conn_curr->llcp_conn_param.state == LLCP_CPR_STATE_RSP_WAIT)) {
            struct connection* conn = _radio.conn_curr;
            struct pdu_data_llctrl_conn_param_req* cpr = (void*)&pdu_data_rx->payload.llctrl.ctrldata.conn_param_req;
    ...
    case PDU_DATA_LLCTRL_TYPE_CONN_PARAM_RSP:
        if (!_radio.conn_curr->role && (_radio.conn_curr->llcp_conn_param.req != _radio.conn_curr->llcp_conn_param.ack) && (_radio.conn_curr->llcp_conn_param.state == LLCP_CPR_STATE_RSP_WAIT)) {
            struct connection* conn = _radio.conn_curr;
            struct pdu_data_llctrl_conn_param_req* cpr = (void*)&pdu_data_rx->payload.llctrl.ctrldata.conn_param_req;
    ...
    case PDU_DATA_LLCTRL_TYPE_UNKNOWN_RSP:
        if (0) {
#if defined(CONFIG_BT_CTLR_DATA_LENGTH)
        } else if (_radio.conn_curr->llcp_length.req != _radio.conn_curr->llcp_length.ack) {
            /* Procedure complete */
            _radio.conn_curr->llcp_length.ack = _radio.conn_curr->llcp_length.req;
            ...
            *rx_enqueue = 1;
...
#if defined(CONFIG_BT_CTLR_PHY)
        } else if (_radio.conn_curr->llcp_phy.req != _radio.conn_curr->llcp_phy.ack) {
            struct radio_le_phy_upd_cmplt* p;
            ...
            llctrl = (struct pdu_data_llctrl*)&pdu_data_rx->payload.llctrl;
    ...
    case PDU_DATA_LLCTRL_TYPE_PHY_REQ:
        if (_radio.role == ROLE_MASTER) {
            if ((_radio.conn_curr->llcp_phy.ack != _radio.conn_curr->llcp_phy.req) && ((_radio.conn_curr->llcp_phy.state == LLCP_PHY_STATE_ACK_WAIT) || (_radio.conn_curr->llcp_phy.state == LLCP_PHY_STATE_RSP_WAIT) || (_radio.conn_curr->llcp_phy.state == LLCP_PHY_STATE_UPD))) {
                /* Same procedure collision  */
                nack = reject_ext_ind_send(_radio.conn_curr,
                    PDU_DATA_LLCTRL_TYPE_PHY_REQ,
                    0x23);
#if defined(CONFIG_BT_CTLR_CONN_PARAM_REQ)
            } else if (((_radio.conn_curr->llcp_req != _radio.conn_curr->llcp_ack) && (_radio.conn_curr->llcp_type != LLCP_ENCRYPTION)) || (_radio.conn_curr->llcp_conn_param.req != _radio.conn_curr->llcp_conn_param.ack)) {
#else /* !CONFIG_BT_CTLR_CONN_PARAM_REQ */
            } else if ((_radio.conn_curr->llcp_req != _radio.conn_curr->llcp_ack) && (_radio.conn_curr->llcp_type != LLCP_ENCRYPTION)) {
#endif /* !CONFIG_BT_CTLR_CONN_PARAM_REQ */
                /* Different procedure collision */
                nack = reject_ext_ind_send(_radio.conn_curr,
                    PDU_DATA_LLCTRL_TYPE_PHY_REQ,
                ...
                struct pdu_data_llctrl* c = &pdu_data_rx->payload.llctrl;
                struct pdu_data_llctrl_phy_req_rsp* p = &c->ctrldata.phy_req;

                _radio.conn_curr->llcp_phy.state = LLCP_PHY_STATE_UPD;

                if (_radio.conn_curr->llcp_phy.ack == _radio.conn_curr->llcp_phy.req) {
                    _radio.conn_curr->llcp_phy.ack--;

                    _radio.conn_curr->llcp_phy.cmd = 0;

                    _radio.conn_curr->llcp_phy.tx = _radio.conn_curr->phy_pref_tx;
                    _radio.conn_curr->llcp_phy.rx = _radio.conn_curr->phy_pref_rx;
                }
    ...
    case PDU_DATA_LLCTRL_TYPE_PHY_RSP:
        if ((_radio.role == ROLE_MASTER) && (_radio.conn_curr->llcp_phy.ack != _radio.conn_curr->llcp_phy.req) && (_radio.conn_curr->llcp_phy.state == LLCP_PHY_STATE_RSP_WAIT)) {
            struct pdu_data_llctrl_phy_req_rsp* p = &pdu_data_rx->payload.llctrl.ctrldata.phy_rsp;
    ...
    case PDU_DATA_LLCTRL_TYPE_MIN_USED_CHAN_IND:
        if (!_radio.conn_curr->role) {
            struct pdu_data_llctrl_min_used_chans_ind* p = &pdu_data_rx->payload.llctrl.ctrldata.min_used_chans_ind;
            struct connection* conn = _radio.conn_curr;
    ...
    struct pdu_data* pdu_data)
{
    ...
    return (pdu_data->ll_id != PDU_DATA_LLID_CTRL) || (!conn->role && ((!conn->refresh && (opcode != PDU_DATA_LLCTRL_TYPE_TERMINATE_IND) && (opcode != PDU_DATA_LLCTRL_TYPE_START_ENC_REQ) && (opcode != PDU_DATA_LLCTRL_TYPE_START_ENC_RSP) && (opcode != PDU_DATA_LLCTRL_TYPE_REJECT_IND) && (opcode != PDU_DATA_LLCTRL_TYPE_REJECT_EXT_IND)) || (conn->refresh && (opcode != PDU_DATA_LLCTRL_TYPE_TERMINATE_IND) && (opcode != PDU_DATA_LLCTRL_TYPE_PAUSE_ENC_RSP) && (opcode != PDU_DATA_LLCTRL_TYPE_ENC_RSP) && (opcode != PDU_DATA_LLCTRL_TYPE_START_ENC_REQ) && (opcode != PDU_DATA_LLCTRL_TYPE_START_ENC_RSP) && (opcode != PDU_DATA_LLCTRL_TYPE_REJECT_IND) && (opcode != PDU_DATA_LLCTRL_TYPE_REJECT_EXT_IND)))) || (conn->role && ((!conn->refresh && (opcode != PDU_DATA_LLCTRL_TYPE_TERMINATE_IND) && (opcode != PDU_DATA_LLCTRL_TYPE_START_ENC_RSP) && (opcode != PDU_DATA_LLCTRL_TYPE_REJECT_IND) && (opcode != PDU_DATA_LLCTRL_TYPE_REJECT_EXT_IND)) || (conn->refresh && (opcode != PDU_DATA_LLCTRL_TYPE_TERMINATE_IND) && (opcode != PDU_DATA_LLCTRL_TYPE_PAUSE_ENC_RSP) && (opcode != PDU_DATA_LLCTRL_TYPE_ENC_REQ) && (opcode != PDU_DATA_LLCTRL_TYPE_START_ENC_RSP) && (opcode != PDU_DATA_LLCTRL_TYPE_REJECT_IND) && (opcode != PDU_DATA_LLCTRL_TYPE_REJECT_EXT_IND))));
}
    ...
    struct radio_pdu_node_tx** tx_release, u8_t* rx_enqueue)
{
    ...
    if (pdu_data_rx->nesn != _radio.conn_curr->sn) {
        ...
        if (_radio.conn_curr->empty == 0) {
            ...
            pdu_data_tx = (struct pdu_data*)(node_tx->pdu_data + _radio.conn_curr->packet_tx_head_offset);
            ...
            if (pdu_data_tx_len != 0) {
                ...
                if (pdu_data_tx->ll_id == PDU_DATA_LLID_CTRL) {
                    terminate = isr_rx_conn_pkt_ack(pdu_data_tx,
                        &node_tx);
                }
            }

            _radio.conn_curr->packet_tx_head_offset += pdu_data_tx_len;
            if (_radio.conn_curr->packet_tx_head_offset == _radio.conn_curr->packet_tx_head_len) {
	 ...
	 */
        (packet_rx_reserve_get(3) != 0) && ((_radio.fc_ena == 0) || ((_radio.link_rx_head == _radio.link_rx_tail) && (_radio.fc_req == _radio.fc_ack)) || ((_radio.link_rx_head != _radio.link_rx_tail) && (_radio.fc_req != _radio.fc_ack) && (((_radio.fc_req == 0) && (_radio.fc_handle[TRIPLE_BUFFER_SIZE - 1] == _radio.conn_curr->handle)) || ((_radio.fc_req != 0) && (_radio.fc_handle[_radio.fc_req - 1] == _radio.conn_curr->handle)))))) {
        ...
        if (pdu_data_rx->len != 0) {
            ...
            if ((_radio.conn_curr->enc_rx && !radio_ccm_mic_is_valid()) || (_radio.conn_curr->pause_rx && isr_rx_conn_enc_unexpected(_radio.conn_curr,
                                                                                                              pdu_data_rx))) {
                _radio.state = STATE_CLOSE;
                radio_disable();
...
#if defined(CONFIG_BT_CTLR_LE_PING)
        } else if ((_radio.conn_curr->enc_rx) || (_radio.conn_curr->pause_rx)) {
            struct connection* conn = _radio.conn_curr;
            u16_t appto_reload_new;

            /* check for change in apto */
            appto_reload_new = (conn->apto_reload > (conn->latency + 6)) ? (conn->apto_reload - (conn->latency + 6)) : conn->apto_reload;
    ...
    u8_t rssi_ready)
{
    ...
    if (crc_ok) {
        ...
        if (!_radio.conn_curr->supervision_expire) {
            _radio.conn_curr->supervision_expire = _radio.conn_curr->supervision_reload;
        }
    }
    ...
    if (SILENT_CONNECTION) {
        ...
        if (!trx_done) {
            ...
            if ((_radio.conn_curr->empty) && (pdu_data_tx->md == 0)) {
                _radio.state = STATE_CLOSE;
                radio_disable();
    ...
    pdu_data_rx = (struct pdu_data*)radio_pdu_node_rx->pdu_data;
    _radio.state = ((_radio.state == STATE_CLOSE) || (crc_close) || ((crc_ok) && (pdu_data_rx->md == 0) && (pdu_data_tx->len == 0)) || _radio.conn_curr->llcp_terminate.reason_peer) ? STATE_CLOSE : STATE_TX;
...
#if defined(CONFIG_BT_CTLR_PHY)
    radio_gpio_pa_lna_enable(radio_tmr_end_get() + RADIO_TIFS - radio_rx_chain_delay_get(
...
isr_rx_conn_exit:
    ...
    if (rssi_ready) {
        ...
        if (((_radio.conn_curr->rssi_reported - rssi) & 0xFF) > RADIO_RSSI_THRESHOLD) {
            if (_radio.conn_curr->rssi_sample_count) {
                _radio.conn_curr->rssi_sample_count--;
            }
        } else {
            _radio.conn_curr->rssi_sample_count = RADIO_RSSI_SAMPLE_COUNT;
        }
    }
    ...
    u8_t rssi_ready)
{
    ...
    if (!((trx_done) || ((SILENT_CONNECTION) && (_radio.role == ROLE_SLAVE)))) {
        _radio.state = STATE_CLOSE;
        radio_disable();
    ...
    case ROLE_SCAN:
#if defined(CONFIG_BT_CTLR_PRIVACY)
        rl_idx = devmatch_ok ? ctrl_rl_idx(!!(_radio.scanner.filter_policy & 0x01),
                                   devmatch_id)
                             : irkmatch_ok ? ctrl_rl_irk_idx(irkmatch_id) : FILTER_IDX_NONE;
#else
        rl_idx = FILTER_IDX_NONE;
#endif
        if (crc_ok && isr_rx_scan_check(irkmatch_ok, devmatch_ok, rl_idx)) {
...
static inline u32_t isr_close_adv(void)
{
    ...
    if ((_radio.state == STATE_CLOSE) && (_radio.advertiser.chan_map_current != 0)) {
        u32_t start_us;
        ...
        radio_gpio_pa_setup();
        radio_gpio_pa_lna_enable(start_us + radio_tx_ready_delay_get(0, 0) - CONFIG_BT_CTLR_GPIO_PA_OFFSET);
#else /* !CONFIG_BT_CTLR_GPIO_PA_PIN */
        ARG_UNUSED(start_us);
        ...
        pdu_adv = (struct pdu_adv*)&_radio.advertiser.adv_data.data[_radio.advertiser.adv_data.first][0];
        if ((_radio.state == STATE_CLOSE) && (pdu_adv->type != PDU_ADV_TYPE_DIRECT_IND)) {
            u32_t ticker_status;
            u8_t random_delay;
            ...
            ticker_status = ticker_update(RADIO_TICKER_INSTANCE_ID_RADIO,
                ...
                (void*)__LINE__);
            LL_ASSERT((ticker_status == TICKER_STATUS_SUCCESS) || (ticker_status == TICKER_STATUS_BUSY) || (_radio.ticker_id_stop == RADIO_TICKER_ID_ADV));
        }
    }
...
static inline u32_t isr_close_scan(void)
{
    ...
    if (_radio.state == STATE_CLOSE) {
        ...
        radio_gpio_lna_setup();
        radio_gpio_pa_lna_enable(start_us + radio_rx_ready_delay_get(0) - CONFIG_BT_CTLR_GPIO_LNA_OFFSET);
...
static inline void isr_close_conn(void)
{
    ...
    if (reason_peer && ((_radio.role == ROLE_SLAVE) || _radio.conn_curr->master.terminate_ack)) {
    ...
    if (_radio.packet_counter && (!SILENT_CONNECTION || (_radio.packet_counter != 0xFF))) {
        if (_radio.role == ROLE_SLAVE) {
            ...
            start_to_address_actual_us = radio_tmr_aa_restore() - radio_tmr_ready_get();
            window_widening_event_us = _radio.conn_curr->slave.window_widening_event_us;
#if defined(CONFIG_BT_CTLR_PHY)
            preamble_to_addr_us = addr_us_get(_radio.conn_curr->phy_rx);
#else /* !CONFIG_BT_CTLR_PHY */
            preamble_to_addr_us = addr_us_get(0);
#endif /* !CONFIG_BT_CTLR_PHY */
            start_to_address_expected_us = RADIO_TICKER_JITTER_US + (RADIO_TICKER_JITTER_US << 1) + preamble_to_addr_us + window_widening_event_us;
            if (start_to_address_actual_us <= start_to_address_expected_us) {
                ticks_drift_plus = TICKER_US_TO_TICKS(window_widening_event_us);
                ticks_drift_minus = TICKER_US_TO_TICKS((u64_t)(start_to_address_expected_us - start_to_address_actual_us));
            } else {
                ticks_drift_plus = TICKER_US_TO_TICKS(start_to_address_actual_us);
                ticks_drift_minus = TICKER_US_TO_TICKS(RADIO_TICKER_JITTER_US + (RADIO_TICKER_JITTER_US << 1) + preamble_to_addr_us);
            }
            ...
            if (_radio.conn_curr->pkt_tx_head) {
                ...
                if (pdu_data_tx->len || _radio.conn_curr->packet_tx_head_offset) {
        ...
        if (_radio.conn_curr->supervision_expire > elapsed_event) {
			 ...
			 */
                ...
                if (latency_event != 0) {
                    ...
                    if (force) {
                        _radio.conn_curr->slave.force |= ((u32_t)1 << 31);
                    }
                }
            }
        } else {
    ...
    if (_radio.conn_curr->appto_expire != 0) {
        if (_radio.conn_curr->appto_expire > elapsed_event) {
            ...
            if ((_radio.conn_curr->procedure_expire == 0) && (_radio.conn_curr->llcp_req == _radio.conn_curr->llcp_ack)) {
    ...
    if (_radio.conn_curr->rssi_sample_count == 0) {
        ...
        radio_pdu_node_rx = packet_rx_reserve_get(2);
        if (radio_pdu_node_rx) {
            _radio.conn_curr->rssi_reported = _radio.conn_curr->rssi_latest;
            _radio.conn_curr->rssi_sample_count = RADIO_RSSI_SAMPLE_COUNT;
            ...
            pdu_data_rx = (struct pdu_data*)
                              radio_pdu_node_rx->pdu_data;
            pdu_data_rx->payload.rssi = _radio.conn_curr->rssi_reported;
    ...
    if ((_radio.conn_curr->llcp_ack != _radio.conn_curr->llcp_req) && ((_radio.conn_curr->llcp_type == LLCP_CONN_UPD) || (_radio.conn_curr->llcp_type == LLCP_CHAN_MAP))) {
        _radio.conn_curr->latency_event = 0;
    }
    ...
    if ((ticks_drift_plus != 0) || (ticks_drift_minus != 0) || (lazy != 0) || (force != 0)) {
        u32_t ticker_status;
        u8_t ticker_id = RADIO_TICKER_ID_FIRST_CONNECTION + _radio.conn_curr->handle;
        ...
        LL_ASSERT((ticker_status == TICKER_STATUS_SUCCESS) || (ticker_status == TICKER_STATUS_BUSY) || (_radio.ticker_id_stop == ticker_id));
    }
}
...
static inline void isr_radio_state_close(void)
{
    ...
    case ROLE_ADV:
        ...
        if (!dont_close) {
            ...
            radio_pdu_node_rx = packet_rx_reserve_get(3);
            if (radio_pdu_node_rx) {
                radio_pdu_node_rx->hdr.type = NODE_RX_TYPE_ADV_INDICATION;
                radio_pdu_node_rx->hdr.handle = 0xFFFF;
    ...
    case ROLE_NONE:
		 ...
		 */
        LL_ASSERT((_radio.state == STATE_STOP) || (_radio.state == STATE_ABORT));
...
static void ticker_update_adv_assert(u32_t status, void* params)
{
    ...
    LL_ASSERT((status == TICKER_STATUS_SUCCESS) || (_radio.ticker_id_stop == RADIO_TICKER_ID_ADV));
}
...
static void ticker_update_slave_assert(u32_t status, void* params)
{
    ...
    LL_ASSERT((status == TICKER_STATUS_SUCCESS) || (_radio.ticker_id_stop == ticker_id));
}
    ...
    u8_t ticker_id)
{
    if (hdr->ticks_xtal_to_start & ((u32_t)1 << 31)) {
        u32_t ticker_status;
        u32_t ticks_prepare_to_start = (hdr->ticks_active_to_start > hdr->ticks_preempt_to_start) ? hdr->ticks_active_to_start : hdr->ticks_preempt_to_start;
        u32_t ticks_drift_minus = (hdr->ticks_xtal_to_start & (~((u32_t)1 << 31))) - ticks_prepare_to_start;
...
{
    u32_t diff = ticker_ticks_diff_get(ticker_ticks_now_get(), ticks_at_expire);
    ...
    if (diff > TICKER_US_TO_TICKS(RADIO_TICKER_START_PART_US)) {
        ...
        if (diff < TICKER_US_TO_TICKS(RADIO_TICKER_PREEMPT_PART_MAX_US)) {
...
static void mayfly_xtal_stop_calc(void* params)
{
    ...
    } while (ticker_id != 0xff && ticker_id >= (RADIO_TICKER_ID_FIRST_CONNECTION + _radio.connection_count));

    if ((ticker_id != 0xff) && (ticks_to_expire < TICKER_US_TO_TICKS(CONFIG_BT_CTLR_XTAL_THRESHOLD))) {
        ...
        if (ticker_id >= RADIO_TICKER_ID_ADV) {
			 ...
			 */
            ticks_prepare_to_start = (hdr->ticks_active_to_start > hdr->ticks_preempt_to_start) ? hdr->ticks_active_to_start : hdr->ticks_preempt_to_start;
            if ((hdr->ticks_xtal_to_start & ((u32_t)1 << 31)) != 0) {
                ticks_to_expire -= ((hdr->ticks_xtal_to_start & (~((u32_t)1 << 31))) - ticks_prepare_to_start);
            } else {
                /* Postpone the primary because we dont have
				 * to start xtal.
				 */
                if (hdr->ticks_xtal_to_start > ticks_prepare_to_start) {
                    u32_t ticks_drift_plus = hdr->ticks_xtal_to_start - ticks_prepare_to_start;
                    u32_t ticker_status;
                    ...
                    ticker_status = ticker_update(
                        ...
                        prepare_reduced,
                        hdr);
                    LL_ASSERT((TICKER_STATUS_SUCCESS == ticker_status) || (TICKER_STATUS_BUSY == ticker_status));
                }
            }
            ...
            if (ticker_id_current >= RADIO_TICKER_ID_FIRST_CONNECTION) {
				 ...
				 * prepare.
				 */
                conn_curr = mem_get(_radio.conn_pool,
                    CONNECTION_T_SIZE,
                    (ticker_id_current - RADIO_TICKER_ID_FIRST_CONNECTION));
                ticks_prepare_to_start = (conn_curr->hdr.ticks_active_to_start > conn_curr->hdr.ticks_preempt_to_start) ? conn_curr->hdr.ticks_active_to_start : conn_curr->hdr.ticks_preempt_to_start;
                if ((conn_curr->hdr.ticks_xtal_to_start & ((u32_t)1 << 31)) != 0) {
                    ticks_to_expire += ((conn_curr->hdr.ticks_xtal_to_start & (~((u32_t)1 << 31))) - ticks_prepare_to_start);
                }
            }
			 ...
			 */
            if ((conn) && (conn_curr) && (conn_curr->conn_interval == conn->conn_interval)) {
                u32_t ticks_conn_interval = TICKER_US_TO_TICKS(conn->conn_interval * 1250);
				 ...
				 * conn param req.
				 */
                if (conn_curr->role && !conn->role && (ticks_to_expire < (TICKER_US_TO_TICKS(RADIO_TICKER_XTAL_OFFSET_US + 625) + conn_curr->hdr.ticks_slot))) {
                    u32_t status;

                    status = conn_update_req(conn_curr);
                    if ((status == 2) && (conn->llcp_version.rx)) {
                        conn_update_req(conn);
                    }
                } else if (!conn_curr->role && conn->role && (ticks_to_expire < (TICKER_US_TO_TICKS(RADIO_TICKER_XTAL_OFFSET_US + 625) + conn_curr->hdr.ticks_slot))) {
                    u32_t status;

                    status = conn_update_req(conn);
                    if ((status == 2) && (conn_curr->llcp_version.rx)) {
    ...
    u32_t* us_offset)
{
    ...
    while (1) {
        ...
        if (ticker_id < RADIO_TICKER_ID_FIRST_CONNECTION || ticker_id >= (RADIO_TICKER_ID_FIRST_CONNECTION + _radio.connection_count)) {
            continue;
        }
        ...
        if (conn && !conn->role) {
            ...
            if (conn->hdr.ticks_xtal_to_start & ((u32_t)1 << 31)) {
                u32_t ticks_prepare_to_start = (conn->hdr.ticks_active_to_start > conn->hdr.ticks_preempt_to_start) ? conn->hdr.ticks_active_to_start : conn->hdr.ticks_preempt_to_start;

                ticks_to_expire_normal -= ((conn->hdr.ticks_xtal_to_start & (~((u32_t)1 << 31))) - ticks_prepare_to_start);
            }

            if ((ticker_id_prev != 0xFF) && (ticker_ticks_diff_get(ticks_to_expire_normal,
                                                 ticks_to_expire_prev)
                                                > (ticks_slot_prev_abs + ticks_slot_abs + TICKER_US_TO_TICKS(RADIO_TICKER_JITTER_US << 2)))) {
                break;
            }

            ticker_id_prev = ticker_id;
            ticks_to_expire_prev = ticks_to_expire_normal;
            ticks_slot_prev_abs = TICKER_US_TO_TICKS(RADIO_TICKER_XTAL_OFFSET_US) + conn->hdr.ticks_slot;
        }
    }

    if (ticker_id_prev != 0xff) {
        *us_offset = TICKER_TICKS_TO_US(ticks_to_expire_prev + ticks_slot_prev_abs) + (RADIO_TICKER_JITTER_US << 1);
    }
}
    ...
    u32_t* win_offset_us)
{
    ...
    if (ticks_anchor_offset != ticks_anchor) {
        *win_offset_us += TICKER_TICKS_TO_US(ticker_ticks_diff_get(ticks_anchor_offset,
            ticks_anchor));
    }
    ...
    u8_t* win_offset)
{
    ...
    if (conn_curr->hdr.ticks_xtal_to_start & ((u32_t)1 << 31)) {
        u32_t ticks_prepare_to_start = (conn_curr->hdr.ticks_active_to_start > conn_curr->hdr.ticks_preempt_to_start) ? conn_curr->hdr.ticks_active_to_start : conn_curr->hdr.ticks_preempt_to_start;

        ticks_prepare_reduced = ((conn_curr->hdr.ticks_xtal_to_start & (~((u32_t)1 << 31))) - ticks_prepare_to_start);
    }

    ticker_id = ticker_id_prev = ticker_id_other = 0xFF;
    ticks_to_expire = ticks_to_expire_prev = ticks_anchor = ticks_anchor_prev = offset_index = _win_offset = 0;
    ticks_slot_prev_abs = 0;
    do {
        u32_t volatile ret_cb = TICKER_STATUS_BUSY;
        struct connection* conn;
        ...
        if ((ticker_id_prev != 0xff) && (ticks_anchor != ticks_anchor_prev)) {
            LL_ASSERT(0);
        }
		 ...
		 */
        if (ticker_id <= RADIO_TICKER_ID_ADV || ticker_id >= (RADIO_TICKER_ID_FIRST_CONNECTION + _radio.connection_count)) {
            continue;
        }
        ...
        if ((conn != conn_curr) && (is_select || !conn->role)) {
            u32_t ticks_to_expire_normal = ticks_to_expire + ticks_prepare_reduced;
            ...
            if (*ticks_to_offset_next < ticks_to_expire_normal) {
                ...
                while ((offset_index < *offset_max) && (ticker_ticks_diff_get(ticks_to_expire_normal,
                                                            ticks_to_expire_prev)
                                                           >= (ticks_slot_prev_abs + TICKER_US_TO_TICKS(RADIO_TICKER_XTAL_OFFSET_US + 625 + 1250) + conn->hdr.ticks_slot))) {
                    _win_offset = TICKER_TICKS_TO_US(ticks_to_expire_prev + ticks_slot_prev_abs) / 1250;
...
static void mayfly_sched_free_win_offset_calc(void* params)
{
    ...
    if (conn->role) {
        conn->llcp_conn_param.ticks_to_offset_next = conn->slave.ticks_to_offset;

        ticks_to_offset_next = &conn->llcp_conn_param.ticks_to_offset_next;
    }
...
#define OFFSET_M_MAX 6
    ...
    ticks_to_offset = TICKER_US_TO_TICKS(conn->llcp_conn_param.offset0 * 1250);
    ...
    while (offset_index_s < OFFSET_S_MAX) {
        ...
        memcpy((u8_t*)&win_offset_s,
            ((u8_t*)&conn->llcp_conn_param.offset0 + (sizeof(u16_t) * offset_index_s)), sizeof(u16_t));

        while (offset_index_m < offset_m_max) {
            if (win_offset_s != 0xffff) {
                if (win_offset_s == win_offset_m[offset_index_m]) {
                    break;
                }
    ...
    if (offset_index_s < OFFSET_S_MAX) {
        conn->llcp.conn_upd.win_offset_us = win_offset_s * 1250;
        memcpy(conn->llcp.conn_upd.pdu_win_offset,
            &win_offset_s, sizeof(u16_t));
    } else if (!has_offset_s) {
        conn->llcp.conn_upd.win_offset_us = win_offset_m[0] * 1250;
        memcpy(conn->llcp.conn_upd.pdu_win_offset,
        ...
        /* send reject_ind_ext */
        pdu_ctrl_tx = (struct pdu_data*)((u8_t*)conn->llcp.conn_upd.pdu_win_offset - offsetof(struct pdu_data,
                                                                                         payload.llctrl.ctrldata.conn_update_ind.win_offset));
        pdu_ctrl_tx->ll_id = PDU_DATA_LLID_CTRL;
        pdu_ctrl_tx->len = offsetof(struct pdu_data_llctrl, ctrldata) + sizeof(struct pdu_data_llctrl_reject_ext_ind);
        pdu_ctrl_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_REJECT_EXT_IND;
        pdu_ctrl_tx->payload.llctrl.ctrldata.reject_ext_ind.reject_opcode = PDU_DATA_LLCTRL_TYPE_CONN_PARAM_REQ;
        pdu_ctrl_tx->payload.llctrl.ctrldata.reject_ext_ind.error_code = 0x20; /* Unsupported parameter value */
    }
}
#endif /* CONFIG_BT_CTLR_CONN_PARAM_REQ */
#endif /* CONFIG_BT_CTLR_SCHED_ADVANCED */
...
static void mayfly_radio_stop(void* params)
{
    ...
    radio_used = ((_radio.state != STATE_NONE) && (_radio.state != STATE_STOP) && (_radio.state != STATE_ABORT));
    ...
    void* context)
{
    ...
    if (_ticks_xtal_to_start & ((u32_t)1 << 31)) {
        _ticks_xtal_to_start = (_ticks_active_to_start > ticks_preempt_to_start) ? _ticks_active_to_start : ticks_preempt_to_start;
    }
...
#undef RADIO_DEFERRED_PREEMPT
    ...
    if (_radio.ticks_active_to_start != _ticks_active_to_start) {
        u32_t ticks_to_start_new = ((_radio.ticks_active_to_start < (*ticks_xtal_to_start & ~(((u32_t)1 << 31)))) ? (*ticks_xtal_to_start & ~(((u32_t)1 << 31))) : _radio.ticks_active_to_start);
...
again:
    ...
    while (bit_idx--) {
		 ...
		 */
        if ((consecutive_cnt > 6) || ((bit_idx < 28) && (transitions < 1)) || ((bit_idx < 27) && (transitions < 2))) {
            if (consecutive_bit) {
                consecutive_bit = 0;
    ...
    /* It shall not have all four octets equal. */
    if (!((access_addr & 0xFFFF) ^ (access_addr >> 16)) && !((access_addr & 0xFF) ^ (access_addr >> 24))) {
        goto again;
    }
...
static void adv_setup(void)
{
    ...
    if (_radio.advertiser.adv_data.first != _radio.advertiser.adv_data.last) {
        u8_t first;
    ...
    if (_radio.advertiser.scan_data.first != _radio.advertiser.scan_data.last) {
        u8_t first;
    ...
    pdu = (struct pdu_adv*)
              _radio.advertiser.adv_data.data[_radio.advertiser.adv_data.first];
#if defined(CONFIG_BT_CTLR_PRIVACY)
    if (upd) {
        struct pdu_adv* scan_pdu = (struct pdu_adv*)
                                       _radio.advertiser.scan_data.data[_radio.advertiser.scan_data.first];
    ...
    if ((pdu->type != PDU_ADV_TYPE_NONCONN_IND) && (!IS_ENABLED(CONFIG_BT_CTLR_ADV_EXT) || (pdu->type != PDU_ADV_TYPE_EXT_IND))) {
        _radio.state = STATE_TX;
        radio_tmr_tifs_set(RADIO_TIFS);
        radio_switch_complete_and_rx(0);
    } else {
    ...
    _radio.advertiser.chan_map_current &= (_radio.advertiser.chan_map_current - 1);
    ...
    u16_t lazy, void* context)
{
...
#if defined(CONFIG_BT_CTLR_PRIVACY)
    if (ctrl_rl_enabled()) {
        struct ll_filter* filter = ctrl_filter_get(!!(_radio.advertiser.filter_policy));
    ...
    remainder_us = radio_tmr_start(1,
        ticks_at_expire + TICKER_US_TO_TICKS(RADIO_TICKER_START_PART_US),
...
#if defined(CONFIG_BT_CTLR_GPIO_PA_PIN)
    radio_gpio_pa_setup();
    radio_gpio_pa_lna_enable(remainder_us + radio_tx_ready_delay_get(0, 0) - CONFIG_BT_CTLR_GPIO_PA_OFFSET);
#else /* !CONFIG_BT_CTLR_GPIO_PA_PIN */
...
/* Ticker Job Silence */
        ...
        ticker_status = ticker_job_idle_get(RADIO_TICKER_INSTANCE_ID_RADIO,
            RADIO_TICKER_USER_ID_WORKER,
...
static void mayfly_adv_stop(void* param)
{
    ...
    /* prepare connection complete structure */
    pdu_data_rx = (struct pdu_data*)radio_pdu_node_rx->pdu_data;
    radio_le_conn_cmplt = (struct radio_le_conn_cmplt*)&pdu_data_rx->payload;
...
<<<<<<< HEAD
{
    ...
    if (ret_cb == TICKER_STATUS_SUCCESS) {
        ...
        if (ret_cb == TICKER_STATUS_SUCCESS) {
			 ...
			 * start
			 */
            if (_radio.advertiser.hdr.ticks_active_to_start > (_radio.advertiser.hdr.ticks_xtal_to_start & ~BIT(31))) {
                u32_t retval;
...
static void ticker_stop_adv_stop(u32_t status, void* params)
{
    ...
    if ((_radio.ticker_id_prepare == RADIO_TICKER_ID_ADV) || (_radio.ticker_id_event == RADIO_TICKER_ID_ADV)) {
	 ...
	 */
        if (_radio.scanner.conn) {
            ...
            if (_radio.scanner.hdr.ticks_xtal_to_start & ((u32_t)1 << 31)) {
                u32_t ticks_prepare_to_start = (_radio.scanner.hdr.ticks_active_to_start > _radio.scanner.hdr.ticks_preempt_to_start) ? _radio.scanner.hdr.ticks_active_to_start : _radio.scanner.hdr.ticks_preempt_to_start;

                ticks_at_expire_normal -= ((_radio.scanner.hdr.ticks_xtal_to_start & (~((u32_t)1 << 31))) - ticks_prepare_to_start);
            }

            s_mfy_sched_after_mstr_free_offset_get.param = (void*)ticks_at_expire_normal;
...
#if defined(CONFIG_BT_CTLR_PRIVACY)
        if (ctrl_rl_enabled()) {
            struct ll_filter* filter = ctrl_filter_get(!!(_radio.scanner.filter_policy & 0x1));
            u8_t count, *irks = ctrl_irks_get(&count);
...
#if defined(CONFIG_BT_CTLR_GPIO_LNA_PIN)
        radio_gpio_lna_setup();
        radio_gpio_pa_lna_enable(remainder_us + radio_rx_ready_delay_get(0) - CONFIG_BT_CTLR_GPIO_LNA_OFFSET);
#else /* !CONFIG_BT_CTLR_GPIO_LNA_PIN */
    ARG_UNUSED(remainder_us);
#endif /* !CONFIG_BT_CTLR_GPIO_LNA_PIN */
...
#endif /* CONFIG_BT_CTLR_XTAL_ADVANCED */
        {
            ...
            ticker_status = ticker_start(RADIO_TICKER_INSTANCE_ID_RADIO,
                ...
                _radio.scanner.ticks_window + TICKER_US_TO_TICKS(RADIO_TICKER_START_PART_US),
                TICKER_NULL_PERIOD, TICKER_NULL_REMAINDER,
                TICKER_NULL_LAZY, TICKER_NULL_SLOT,
...
/* Ticker Job Silence */
        ...
        void (*fp_mayfly_select_or_use)(void*))
    {
        ...
        pdu_ctrl_tx->ll_id = PDU_DATA_LLID_CTRL;
        pdu_ctrl_tx->len = offsetof(struct pdu_data_llctrl, ctrldata) + sizeof(struct pdu_data_llctrl_conn_update_ind);
        pdu_ctrl_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_CONN_UPDATE_IND;
        pdu_ctrl_tx->payload.llctrl.ctrldata.conn_update_ind.win_size = conn->llcp.conn_upd.win_size;
        pdu_ctrl_tx->payload.llctrl.ctrldata.conn_update_ind.win_offset = conn->llcp.conn_upd.win_offset_us / 1250;
        pdu_ctrl_tx->payload.llctrl.ctrldata.conn_update_ind.interval = conn->llcp.conn_upd.interval;
        pdu_ctrl_tx->payload.llctrl.ctrldata.conn_update_ind.latency = conn->llcp.conn_upd.latency;
        pdu_ctrl_tx->payload.llctrl.ctrldata.conn_update_ind.timeout = conn->llcp.conn_upd.timeout;
        pdu_ctrl_tx->payload.llctrl.ctrldata.conn_update_ind.instant = conn->llcp.conn_upd.instant;
		 ...
		 */
            ...
            if (conn->hdr.ticks_xtal_to_start & ((u32_t)1 << 31)) {
                ...
                conn->llcp.conn_upd.ticks_anchor -= ((conn->hdr.ticks_xtal_to_start & (~((u32_t)1 << 31))) - ticks_prepare_to_start);
            }

            conn->llcp.conn_upd.pdu_win_offset = (u16_t*)&pdu_ctrl_tx->payload.llctrl.ctrldata.conn_update_ind.win_offset;
        ...
        u32_t ticks_at_expire)
    {
        ...
        if (conn->llcp.conn_upd.state != LLCP_CUI_STATE_INPROG) {
            ...
            case LLCP_CUI_STATE_SELECT:
                fp_mayfly_select_or_use = mayfly_sched_win_offset_select;
                break;
#endif /* CONFIG_BT_CTLR_CONN_PARAM_REQ */
...
#if defined(CONFIG_BT_CTLR_CONN_PARAM_REQ)
            if ((conn->llcp_conn_param.req != conn->llcp_conn_param.ack) && (conn->llcp_conn_param.state == LLCP_CPR_STATE_UPD)) {
                conn->llcp_conn_param.ack = conn->llcp_conn_param.req;
		 ...
		 */
            ...
            /* Prepare the rx packet structure */
            if ((conn->llcp.conn_upd.interval != conn->conn_interval) || (conn->llcp.conn_upd.latency != conn->latency) || (RADIO_CONN_EVENTS(conn->llcp.conn_upd.timeout * 10000,
                                                                                                                                conn->conn_interval * 1250)
                                                                                                                               != conn->supervision_reload)) {
                radio_pdu_node_rx = packet_rx_reserve_get(2);
                LL_ASSERT(radio_pdu_node_rx);
                ...
                /* prepare connection update complete structure */
                pdu_data_rx = (struct pdu_data*)radio_pdu_node_rx->pdu_data;
                radio_le_conn_update_cmplt = (struct radio_le_conn_update_cmplt*)&pdu_data_rx->payload;
                radio_le_conn_update_cmplt->status = 0x00;
                radio_le_conn_update_cmplt->interval = conn->llcp.conn_upd.interval;
                radio_le_conn_update_cmplt->latency = conn->llcp.conn_upd.latency;
                radio_le_conn_update_cmplt->timeout = conn->llcp.conn_upd.timeout;
            ...
            if (conn->hdr.ticks_xtal_to_start & ((u32_t)1 << 31)) {
                ...
                ticks_at_expire -= (conn->hdr.ticks_xtal_to_start - ticks_prepare_to_start);
            }

            /* compensate for instant_latency due to laziness */
            conn_interval_old = instant_latency * conn->conn_interval;
            latency = conn_interval_old / conn->llcp.conn_upd.interval;
            conn_interval_new = latency * conn->llcp.conn_upd.interval;
            if (conn_interval_new > conn_interval_old) {
                ticks_at_expire += TICKER_US_TO_TICKS((conn_interval_new - conn_interval_old) * 1250);
            } else {
                ticks_at_expire -= TICKER_US_TO_TICKS((conn_interval_old - conn_interval_new) * 1250);
            }
            ...
            if (conn->role) {
                conn->slave.window_widening_prepare_us -= conn->slave.window_widening_periodic_us * instant_latency;

                conn->slave.window_widening_periodic_us = (((gc_lookup_ppm[_radio.sca] + gc_lookup_ppm[conn->slave.sca]) * conn_interval_us) + (1000000 - 1)) / 1000000;
                conn->slave.window_widening_max_us = (conn_interval_us >> 1) - RADIO_TIFS;
                conn->slave.window_size_prepare_us = conn->llcp.conn_upd.win_size * 1250;
                conn->slave.ticks_to_offset = 0;

                conn->slave.window_widening_prepare_us += conn->slave.window_widening_periodic_us * latency;
                if (conn->slave.window_widening_prepare_us > conn->slave.window_widening_max_us) {
                    conn->slave.window_widening_prepare_us = conn->slave.window_widening_max_us;
                }

                ticks_at_expire -= TICKER_US_TO_TICKS(conn->slave.window_widening_periodic_us * latency);
                ticks_win_offset = TICKER_US_TO_TICKS((conn->llcp.conn_upd.win_offset_us / 1250) * 1250);
                periodic_us -= conn->slave.window_widening_periodic_us;
            } else {
                ticks_win_offset = TICKER_US_TO_TICKS(conn->llcp.conn_upd.win_offset_us);
            ...
            conn->supervision_reload = RADIO_CONN_EVENTS((conn->llcp.conn_upd.timeout
                                                             * 10 * 1000),
                conn_interval_us);
		 ...
		 */
            mayfly_was_enabled = mayfly_is_enabled(RADIO_TICKER_USER_ID_WORKER,
                RADIO_TICKER_USER_ID_JOB);
            ...
            ticker_status = ticker_stop(RADIO_TICKER_INSTANCE_ID_RADIO,
                RADIO_TICKER_USER_ID_WORKER,
                RADIO_TICKER_ID_FIRST_CONNECTION + conn->handle, ticker_success_assert,
                (void*)__LINE__);
            LL_ASSERT((ticker_status == TICKER_STATUS_SUCCESS) || (ticker_status == TICKER_STATUS_BUSY));
            ticker_status = ticker_start(RADIO_TICKER_INSTANCE_ID_RADIO,
                RADIO_TICKER_USER_ID_WORKER,
        ...
        u16_t event_counter)
    {
        if (conn->llcp.chan_map.initiate) {
            ...
            node_tx = mem_acquire(&_radio.pkt_tx_ctrl_free);
            if (node_tx) {
                struct pdu_data* pdu_ctrl_tx = (struct pdu_data*)node_tx->pdu_data;

                /* reset initiate flag */
                conn->llcp.chan_map.initiate = 0;

                /* set instant */
                conn->llcp.chan_map.instant = event_counter + conn->latency + 6;
			 ...
			 */
                ...
                pdu_ctrl_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_CHAN_MAP_IND;
                memcpy(&pdu_ctrl_tx->payload.llctrl.ctrldata.chan_map_ind.chm[0],
                    &conn->llcp.chan_map.chm[0],
                    sizeof(pdu_ctrl_tx->payload.llctrl.ctrldata.chan_map_ind.chm));
                pdu_ctrl_tx->payload.llctrl.ctrldata.chan_map_ind.instant = conn->llcp.chan_map.instant;
        ...
        struct pdu_data * pdu)
    {
        ...
        if (conn->common.fex_valid && (conn->llcp_features & BIT(BT_LE_FEAT_BIT_EXT_REJ_IND))) {
            struct pdu_data_llctrl_reject_ext_ind* p;

            pdu->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_REJECT_EXT_IND;
...
/* slave send reject ind or start enc req at control priority */
			 ...
			 */
                ...
                pdu_ctrl_tx->len = offsetof(struct pdu_data_llctrl,
                    ctrldata);
                pdu_ctrl_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_START_ENC_REQ;
            }
    ...
    static inline void event_fex_prep(struct connection * conn)
    {
        ...
        if (node_tx) {
            ...
            pdu_ctrl_tx->ll_id = PDU_DATA_LLID_CTRL;
            pdu_ctrl_tx->len = offsetof(struct pdu_data_llctrl, ctrldata) + sizeof(struct pdu_data_llctrl_feature_req);
            pdu_ctrl_tx->payload.llctrl.opcode = !conn->role ? PDU_DATA_LLCTRL_TYPE_FEATURE_REQ : PDU_DATA_LLCTRL_TYPE_SLAVE_FEATURE_REQ;
            ...
            pdu_ctrl_tx->payload.llctrl.ctrldata.feature_req.features[0] = conn->llcp_features & 0xFF;
            pdu_ctrl_tx->payload.llctrl.ctrldata.feature_req.features[1] = (conn->llcp_features >> 8) & 0xFF;
            pdu_ctrl_tx->payload.llctrl.ctrldata.feature_req.features[2] = (conn->llcp_features >> 16) & 0xFF;
    ...
    static inline void event_vex_prep(struct connection * conn)
    {
        ...
        if (conn->llcp_version.tx == 0) {
            ...
            if (node_tx) {
                ...
                pdu_ctrl_tx->payload.llctrl.ctrldata.version_ind.version_number = RADIO_BLE_VERSION_NUMBER;
                pdu_ctrl_tx->payload.llctrl.ctrldata.version_ind.company_id = RADIO_BLE_COMPANY_ID;
                pdu_ctrl_tx->payload.llctrl.ctrldata.version_ind.sub_version_number = RADIO_BLE_SUB_VERSION_NUMBER;
            ...
            pdu_ctrl_rx->len = offsetof(struct pdu_data_llctrl, ctrldata) + sizeof(struct pdu_data_llctrl_version_ind);
            pdu_ctrl_rx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_VERSION_IND;
            pdu_ctrl_rx->payload.llctrl.ctrldata.version_ind.version_number = conn->llcp_version.version_number;
            pdu_ctrl_rx->payload.llctrl.ctrldata.version_ind.company_id = conn->llcp_version.company_id;
            pdu_ctrl_rx->payload.llctrl.ctrldata.version_ind.sub_version_number = conn->llcp_version.sub_version_number;
        ...
        u32_t ticks_at_expire)
    {
        ...
        pdu_ctrl_tx = (struct pdu_data*)node_tx->pdu_data;
        pdu_ctrl_tx->ll_id = PDU_DATA_LLID_CTRL;
        pdu_ctrl_tx->len = offsetof(struct pdu_data_llctrl, ctrldata) + sizeof(struct pdu_data_llctrl_conn_param_req);
        pdu_ctrl_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_CONN_PARAM_REQ;
	 ...
	 */
...
#if defined(CONFIG_BT_CTLR_SCHED_ADVANCED)
        {
            ...
            if (conn->hdr.ticks_xtal_to_start & ((u32_t)1 << 31)) {
                ...
                conn->llcp_conn_param.ticks_ref -= ((conn->hdr.ticks_xtal_to_start & (~((u32_t)1 << 31))) - ticks_prepare_to_start);
            }
    ...
    static inline void event_conn_param_rsp(struct connection * conn)
    {
        ...
        if (conn->llcp_conn_param.status) {
            ...
            pdu->ll_id = PDU_DATA_LLID_CTRL;
            pdu->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_REJECT_EXT_IND;
            pdu->len = offsetof(struct pdu_data_llctrl, ctrldata) + sizeof(struct pdu_data_llctrl_reject_ext_ind);
        ...
        pdu = (struct pdu_data*)node_tx->pdu_data;
        pdu->ll_id = PDU_DATA_LLID_CTRL;
        pdu->len = offsetof(struct pdu_data_llctrl, ctrldata) + sizeof(struct pdu_data_llctrl_conn_param_rsp);
        pdu->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_CONN_PARAM_RSP;
        ...
        p->reference_conn_event_count = conn->llcp_conn_param.reference_conn_event_count;
    ...
    static inline void event_conn_param_app_req(struct connection * conn)
    {
        ...
        pdu->ll_id = PDU_DATA_LLID_CTRL;
        pdu->len = offsetof(struct pdu_data_llctrl, ctrldata) + sizeof(struct pdu_data_llctrl_conn_param_req);
        pdu->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_CONN_PARAM_REQ;
    ...
    static inline void event_ping_prep(struct connection * conn)
    {
        ...
        if (node_tx) {
            ...
            pdu_ctrl_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_PING_REQ;
    ...
    static inline void event_len_prep(struct connection * conn)
    {
        ...
        case LLCP_LENGTH_STATE_REQ: {
            ...
            free_count_rx = packet_rx_acquired_count_get() + mem_free_count_get(_radio.pkt_rx_data_free);
            LL_ASSERT(free_count_rx <= 0xFF);
            ...
            pdu_ctrl_tx = (struct pdu_data*)node_tx->pdu_data;
            pdu_ctrl_tx->ll_id = PDU_DATA_LLID_CTRL;
            pdu_ctrl_tx->len = offsetof(struct pdu_data_llctrl, ctrldata) + sizeof(struct pdu_data_llctrl_length_req_rsp);
            pdu_ctrl_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_LENGTH_REQ;

            lr = (struct pdu_data_llctrl_length_req_rsp*)&pdu_ctrl_tx->payload.llctrl.ctrldata.length_req;
		 ...
		 */
            if (((free_count_conn + 1) == _radio.connection_count) || (packet_rx_data_size > _radio.packet_rx_data_size)) {
                /* as rx mem is to be re-sized, release acquired
			 * memq link.
			 */
                while (_radio.packet_rx_acquire != _radio.packet_rx_last) {

                    struct radio_pdu_node_rx* node_rx;

                    if (_radio.packet_rx_acquire == 0) {
                        _radio.packet_rx_acquire = _radio.packet_rx_count - 1;
                    } else {
                        _radio.packet_rx_acquire -= 1;
                    }

                    node_rx = _radio.packet_rx[_radio.packet_rx_acquire];
                    mem_release(node_rx->hdr.onion.link,
                        &_radio.link_rx_free);

                    LL_ASSERT(_radio.link_rx_data_quota < (_radio.packet_rx_count - 1));
                ...
                if (conn->max_rx_octets < (PDU_AC_SIZE_MAX + PDU_AC_SIZE_EXTRA)) {
                    _radio.packet_rx_data_size = MROUND(offsetof(struct radio_pdu_node_rx,
                                                            pdu_data)
                        + (PDU_AC_SIZE_MAX + PDU_AC_SIZE_EXTRA));
                } else {
                    _radio.packet_rx_data_size = packet_rx_data_size;
                }
                _radio.packet_rx_data_count = _radio.packet_rx_data_pool_size / _radio.packet_rx_data_size;
            ...
            pdu_ctrl_rx->len = offsetof(struct pdu_data_llctrl,
                                   ctrldata)
                + sizeof(struct pdu_data_llctrl_length_req_rsp);
            pdu_ctrl_rx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_LENGTH_RSP;

            lr = (struct pdu_data_llctrl_length_req_rsp*)&pdu_ctrl_rx->payload.llctrl.ctrldata.length_req;
            lr->max_rx_octets = conn->max_rx_octets;
            lr->max_tx_octets = conn->max_tx_octets;
#if !defined(CONFIG_BT_CTLR_PHY)
            lr->max_rx_time = RADIO_PKT_TIME(conn->max_rx_octets, 0);
    ...
    static inline void event_phy_req_prep(struct connection * conn)
    {
        ...
        case LLCP_PHY_STATE_REQ: {
            ...
            pdu_ctrl_tx = (struct pdu_data*)node_tx->pdu_data;
            pdu_ctrl_tx->ll_id = PDU_DATA_LLID_CTRL;
            pdu_ctrl_tx->len = offsetof(struct pdu_data_llctrl, ctrldata) + sizeof(struct pdu_data_llctrl_phy_req_rsp);
            pdu_ctrl_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_PHY_REQ;

            pr = (struct pdu_data_llctrl_phy_req_rsp*)&pdu_ctrl_tx->payload.llctrl.ctrldata.phy_req;
        ...
        if (conn->llcp.phy_upd_ind.initiate) {
            ...
            if (node_tx) {
                ...
                if (!((conn->llcp.phy_upd_ind.tx | conn->llcp.phy_upd_ind.rx) & 0x07)) {
                    ...
                    /* set instant */
                    conn->llcp.phy_upd_ind.instant = event_counter + conn->latency + 6;
                }
			 ...
			 */
                ...
                pdu_ctrl_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_PHY_UPD_IND;
            ...
            if (!conn->llcp.phy_upd_ind.cmd && (conn->phy_tx == old_tx) && (conn->phy_rx == old_rx)) {
                return;
            }
        ...
        struct connection * conn)
    {
        ...
        LL_ASSERT(!_radio.ticker_id_prepare);
        _radio.ticker_id_prepare = RADIO_TICKER_ID_FIRST_CONNECTION + conn->handle;

        /* Calc window widening */
        if (conn->role) {
            conn->slave.window_widening_prepare_us += conn->slave.window_widening_periodic_us * (lazy + 1);
            if (conn->slave.window_widening_prepare_us > conn->slave.window_widening_max_us) {
                conn->slave.window_widening_prepare_us = conn->slave.window_widening_max_us;
            }
        }
		 ...
		 */
            ...
            if (node_tx) {
                ...
                pdu_ctrl_tx->ll_id = PDU_DATA_LLID_CTRL;
                pdu_ctrl_tx->len = offsetof(struct pdu_data_llctrl, ctrldata) + sizeof(struct pdu_data_llctrl_terminate_ind);
                pdu_ctrl_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_TERMINATE_IND;
                pdu_ctrl_tx->payload.llctrl.ctrldata.terminate_ind.error_code = conn->llcp_terminate.reason_own;
    ...
    static void connection_configure(struct connection * conn)
    {
        ...
        radio_crc_configure(((0x5bUL) | ((0x06UL) << 8) | ((0x00UL) << 16)),
            (((u32_t)conn->crc_init[2] << 16) | ((u32_t)conn->crc_init[1] << 8) | ((u32_t)conn->crc_init[0])));
    }
        ...
        void* context)
    {
        ...
        LL_ASSERT(_radio.ticker_id_prepare == (RADIO_TICKER_ID_FIRST_CONNECTION + conn->handle));

        _radio.role = ROLE_SLAVE;
        _radio.state = STATE_RX;
        _radio.ticker_id_prepare = 0;
        _radio.ticker_id_event = (RADIO_TICKER_ID_FIRST_CONNECTION + conn->handle);
        _radio.ticks_anchor = ticks_at_expire;
    ...
    radio_switch_complete_and_tx(0, 0, 0, 0);
        ...
        rx_packet_set(conn, (struct pdu_data*)_radio.packet_rx[_radio.packet_rx_last]->pdu_data);
        ...
        conn->slave.window_widening_event_us += conn->slave.window_widening_prepare_us;
        conn->slave.window_widening_prepare_us = 0;
        if (conn->slave.window_widening_event_us > conn->slave.window_widening_max_us) {
            conn->slave.window_widening_event_us = conn->slave.window_widening_max_us;
        }
        ...
        remainder_us = radio_tmr_start(0, ticks_at_expire + TICKER_US_TO_TICKS(RADIO_TICKER_START_PART_US),
            _radio.remainder_anchor);

        radio_tmr_aa_capture();
        radio_tmr_aa_save(0);

        hcto = remainder_us + RADIO_TICKER_JITTER_US + (RADIO_TICKER_JITTER_US << 2) + (conn->slave.window_widening_event_us << 1) + conn->slave.window_size_event_us;
...
#if defined(CONFIG_BT_CTLR_PHY)
        radio_gpio_pa_lna_enable(remainder_us + radio_rx_ready_delay_get(conn->phy_rx) - CONFIG_BT_CTLR_GPIO_LNA_OFFSET);
#else /* !CONFIG_BT_CTLR_PHY */
        radio_gpio_pa_lna_enable(remainder_us + radio_rx_ready_delay_get(0) - CONFIG_BT_CTLR_GPIO_LNA_OFFSET);
#endif /* !CONFIG_BT_CTLR_PHY */
#endif /* CONFIG_BT_CTLR_GPIO_LNA_PIN */
...
#if (defined(CONFIG_BT_CTLR_XTAL_ADVANCED) && (RADIO_TICKER_PREEMPT_PART_US <= RADIO_TICKER_PREEMPT_PART_MIN_US))
        /* check if preempt to start has changed */
        if (preempt_calc(&conn->hdr, (RADIO_TICKER_ID_FIRST_CONNECTION + conn->handle), ticks_at_expire) != 0) {
            _radio.state = STATE_STOP;
            radio_disable();
        } else
#endif /* CONFIG_BT_CTLR_XTAL_ADVANCED */
...
/* normal connection! */
#if SILENT_CONNECTION
        if (!conn->empty || pdu_data_tx->md || (conn->supervision_expire && (conn->supervision_expire <= 6)) || (conn->connect_expire && (conn->connect_expire <= 6))) {
#endif
            remainder_us = radio_tmr_start(1,
                ticks_at_expire + TICKER_US_TO_TICKS(RADIO_TICKER_START_PART_US),
                _radio.remainder_anchor);
...
#if defined(CONFIG_BT_CTLR_PHY)
            radio_gpio_pa_lna_enable(remainder_us + radio_tx_ready_delay_get(conn->phy_tx,
                                                        conn->phy_flags)
                - CONFIG_BT_CTLR_GPIO_PA_OFFSET);
#else /* !CONFIG_BT_CTLR_PHY */
            radio_gpio_pa_lna_enable(remainder_us + radio_tx_ready_delay_get(0, 0) - CONFIG_BT_CTLR_GPIO_PA_OFFSET);
...
#if (defined(CONFIG_BT_CTLR_XTAL_ADVANCED) && (RADIO_TICKER_PREEMPT_PART_US <= RADIO_TICKER_PREEMPT_PART_MIN_US))
        /* check if preempt to start has changed */
        if (0 != preempt_calc(&conn->hdr, (RADIO_TICKER_ID_FIRST_CONNECTION + conn->handle), ticks_at_expire)) {
            _radio.state = STATE_STOP;
            radio_disable();
        } else
#endif /* CONFIG_BT_CTLR_XTAL_ADVANCED */
...
/* Ticker Job Silence */
        ...
        struct pdu_data * *pdu_data_tx)
    {
        ...
        if (/* empty packet */
            ...
            _pdu_data_tx = (void*)(conn->pkt_tx_head->pdu_data + conn->packet_tx_head_offset);
            ...
            _pdu_data_tx->len = conn->packet_tx_head_len - conn->packet_tx_head_offset;
            _pdu_data_tx->md = 0;
            ...
            case BIT(2):
				 ...
				 */
                    max_tx_octets = ((conn->max_tx_time - 592) >> 6) - 2;
                } else {
				 ...
				 * Subtract 2 bytes for header.
				 */
                    max_tx_octets = ((conn->max_tx_time - 430) >> 4) - 2;
                }
                break;
#endif /* CONFIG_BT_CTLR_PHY_CODED */
            }
        ...
        max_tx_octets = RADIO_LL_LENGTH_OCTETS_RX_MIN;
            ...
            if (!conn->pkt_tx_ctrl && (conn->pkt_tx_head != conn->pkt_tx_data)) {
    ...
    static inline u8_t packet_rx_acquired_count_get(void)
    {
        if (_radio.packet_rx_acquire >= _radio.packet_rx_last) {
            ...
            return (_radio.packet_rx_count - _radio.packet_rx_last + _radio.packet_rx_acquire);
        }
    }
...
/* Inline call of callback. If JOB configured as lower priority then
	 ...
	 */
        radio_pdu_node_rx->hdr.onion.packet_release_last = _radio.packet_release_last;
	 ...
	 */
        ...
        if ((_radio.state == STATE_ABORT) && (_radio.ticker_id_prepare == (RADIO_TICKER_ID_FIRST_CONNECTION + conn->handle))) {
        ...
        while ((conn->pkt_tx_head != conn->pkt_tx_ctrl) && (conn->pkt_tx_head != conn->pkt_tx_data)) {
            struct radio_pdu_node_tx* node_tx;
            struct pdu_data* pdu_data_tx;
        ...
        while ((conn->pkt_tx_head) && (conn->pkt_tx_head != conn->pkt_tx_data)) {
            void* release;
    ...
    static u32_t conn_update(struct connection * conn, struct pdu_data * pdu_data_rx)
    {
        if (((pdu_data_rx->payload.llctrl.ctrldata.conn_update_ind.instant - conn->event_counter) & 0xFFFF) > 0x7FFF) {
            return 1;
        }
	 ...
	 */
        ...
        conn->llcp.conn_upd.win_size = pdu_data_rx->payload.llctrl.ctrldata.conn_update_ind.win_size;
        conn->llcp.conn_upd.win_offset_us = pdu_data_rx->payload.llctrl.ctrldata.conn_update_ind.win_offset * 1250;
        conn->llcp.conn_upd.interval = pdu_data_rx->payload.llctrl.ctrldata.conn_update_ind.interval;
        conn->llcp.conn_upd.latency = pdu_data_rx->payload.llctrl.ctrldata.conn_update_ind.latency;
        conn->llcp.conn_upd.timeout = pdu_data_rx->payload.llctrl.ctrldata.conn_update_ind.timeout;
        conn->llcp.conn_upd.instant = pdu_data_rx->payload.llctrl.ctrldata.conn_update_ind.instant;
        conn->llcp.conn_upd.state = LLCP_CUI_STATE_INPROG;
...
#if defined(CONFIG_BT_CTLR_CONN_PARAM_REQ)
        if ((conn->llcp_conn_param.req != conn->llcp_conn_param.ack) && (conn->llcp_conn_param.state == LLCP_CPR_STATE_RSP_WAIT)) {
            conn->llcp_conn_param.ack = conn->llcp_conn_param.req;
        }
#endif /* CONFIG_BT_CTLR_CONN_PARAM_REQ */
    ...
    static u32_t conn_update_req(struct connection * conn)
    {
        ...
        if (!conn->role) {
            ...
            conn->llcp.conn_upd.timeout = conn->conn_interval * conn->supervision_reload * 125 / 1000;
            /* conn->llcp.conn_upd.instant     = 0; */
            conn->llcp.conn_upd.state = LLCP_CUI_STATE_USE;
...
#if defined(CONFIG_BT_CTLR_CONN_PARAM_REQ)
        } else if (conn->common.fex_valid && (conn->llcp_features & BIT(BT_LE_FEAT_BIT_CONN_PARAM_REQ))) {
            ...
            conn->llcp_conn_param.timeout = conn->conn_interval * conn->supervision_reload * 125 / 1000;
    ...
    static u32_t chan_map_update(struct connection * conn,
        struct pdu_data * pdu_data_rx)
    {
        if (((pdu_data_rx->payload.llctrl.ctrldata.chan_map_ind.instant - conn->event_counter) & 0xffff) > 0x7fff) {
            return 1;
        }
        ...
        memcpy(&conn->llcp.chan_map.chm[0],
            ...
            sizeof(conn->llcp.chan_map.chm));
        conn->llcp.chan_map.instant = pdu_data_rx->payload.llctrl.ctrldata.chan_map_ind.instant;
        conn->llcp.chan_map.initiate = 0;
        ...
        u8_t * rx_enqueue)
    {
        ...
        if (!((p->m_to_s_phy | p->s_to_m_phy) & 0x07)) {
            ...
            if ((conn->llcp_phy.ack == conn->llcp_phy.req) || (conn->llcp_phy.state != LLCP_PHY_STATE_RSP_WAIT)) {
                return 0;
            }
        ...
        struct radio_pdu_node_tx * node_tx)
    {
        ...
        pdu_ctrl_tx->payload.llctrl.ctrldata.enc_req.ediv[0] = conn->llcp.encryption.ediv[0];
        pdu_ctrl_tx->payload.llctrl.ctrldata.enc_req.ediv[1] = conn->llcp.encryption.ediv[1];
        ...
        struct pdu_data * pdu_ctrl_tx)
    {
        ...
        pdu_ctrl_tx->ll_id = PDU_DATA_LLID_CTRL;
        pdu_ctrl_tx->len = offsetof(struct pdu_data_llctrl, ctrldata);
        pdu_ctrl_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_START_ENC_RSP;
    ...
    static u8_t pause_enc_rsp_send(struct connection * conn, u8_t req)
    {
        ...
        pdu_ctrl_tx->ll_id = PDU_DATA_LLID_CTRL;
        pdu_ctrl_tx->len = offsetof(struct pdu_data_llctrl, ctrldata);
        pdu_ctrl_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_PAUSE_ENC_RSP;
        ...
        struct pdu_data * pdu_data_rx)
    {
        ...
        pdu_ctrl_tx = (struct pdu_data*)node_tx->pdu_data;
        pdu_ctrl_tx->ll_id = PDU_DATA_LLID_CTRL;
        pdu_ctrl_tx->len = offsetof(struct pdu_data_llctrl, ctrldata) + sizeof(struct pdu_data_llctrl_feature_rsp);
        ...
        struct pdu_data * pdu_data_rx, u8_t * rx_enqueue)
    {
        ...
        if (!conn->llcp_version.tx) {
            ...
            pdu_ctrl_tx = (struct pdu_data*)node_tx->pdu_data;
            pdu_ctrl_tx->ll_id = PDU_DATA_LLID_CTRL;
            pdu_ctrl_tx->len = offsetof(struct pdu_data_llctrl, ctrldata) + sizeof(struct pdu_data_llctrl_version_ind);
            pdu_ctrl_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_VERSION_IND;
        ...
        u8_t reject_opcode, u8_t error_code)
    {
        ...
        pdu_ctrl_tx->len = offsetof(struct pdu_data_llctrl, ctrldata) + sizeof(struct pdu_data_llctrl_reject_ext_ind);
        pdu_ctrl_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_REJECT_EXT_IND;
        pdu_ctrl_tx->payload.llctrl.ctrldata.reject_ext_ind.reject_opcode = reject_opcode;
        pdu_ctrl_tx->payload.llctrl.ctrldata.reject_ext_ind.error_code = error_code;
...
#endif /* CONFIG_BT_CTLR_PHY */
    {
        ...
        pdu_ctrl_tx->ll_id = PDU_DATA_LLID_CTRL;
        pdu_ctrl_tx->len = offsetof(struct pdu_data_llctrl, ctrldata) + sizeof(struct pdu_data_llctrl_length_req_rsp);
        pdu_ctrl_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_LENGTH_RSP;
        pdu_ctrl_tx->payload.llctrl.ctrldata.length_rsp.max_rx_octets = eff_rx_octets;
        pdu_ctrl_tx->payload.llctrl.ctrldata.length_rsp.max_tx_octets = eff_tx_octets;

#if !defined(CONFIG_BT_CTLR_PHY)
        pdu_ctrl_tx->payload.llctrl.ctrldata.length_rsp.max_rx_time = RADIO_PKT_TIME(eff_rx_octets, 0);
        pdu_ctrl_tx->payload.llctrl.ctrldata.length_rsp.max_tx_time = RADIO_PKT_TIME(eff_tx_octets, 0);
#else /* CONFIG_BT_CTLR_PHY */
        pdu_ctrl_tx->payload.llctrl.ctrldata.length_rsp.max_rx_time = eff_rx_time;
        pdu_ctrl_tx->payload.llctrl.ctrldata.length_rsp.max_tx_time = eff_tx_time;
#endif /* CONFIG_BT_CTLR_PHY */
    ...
    static u8_t phy_rsp_send(struct connection * conn, struct pdu_data * pdu_data_rx)
    {
        ...
        pdu_ctrl_tx->payload.llctrl.ctrldata.phy_rsp.tx_phys = conn->phy_pref_tx;
        pdu_ctrl_tx->payload.llctrl.ctrldata.phy_rsp.rx_phys = conn->phy_pref_rx;
        ...
        u32_t ticks_active_to_start)
    {
		 ...
		 */
            if (ticker_id_stop != TICKER_NULL) {
                ...
                LL_ASSERT((ret_cb == TICKER_STATUS_SUCCESS) || (ret_cb == TICKER_STATUS_FAILURE));
            }
	 ...
	 */
        ...
        case RADIO_TICKER_ID_ADV:
            ...
            ticks_active_to_start = _radio.advertiser.hdr.ticks_active_to_start;
        ...
        case RADIO_TICKER_ID_SCAN:
            ...
            ticks_active_to_start = _radio.scanner.hdr.ticks_active_to_start;
            break;
        default:
            if (ticker_id_primary >= RADIO_TICKER_ID_FIRST_CONNECTION) {
                ...
                conn_handle = ticker_id_primary - RADIO_TICKER_ID_FIRST_CONNECTION;
...
u32_t radio_adv_enable(u16_t interval, u8_t chan_map, u8_t filter_policy,
...
#endif /* !CONFIG_BT_CTLR_ADV_EXT */
    {
        ...
        pdu_adv = (struct pdu_adv*)&_radio.advertiser.adv_data.data[_radio.advertiser.adv_data.last][0];

        if ((pdu_adv->type == PDU_ADV_TYPE_ADV_IND) || (pdu_adv->type == PDU_ADV_TYPE_DIRECT_IND)) {
            void* link;
    ...
    ARG_UNUSED(rl_idx);
        ...
        _radio.advertiser.hdr.ticks_active_to_start = _radio.ticks_active_to_start;
        _radio.advertiser.hdr.ticks_xtal_to_start = TICKER_US_TO_TICKS(RADIO_TICKER_XTAL_OFFSET_US);
        _radio.advertiser.hdr.ticks_preempt_to_start = TICKER_US_TO_TICKS(RADIO_TICKER_PREEMPT_PART_MIN_US);

        chan_cnt = util_ones_count_get(&chan_map, sizeof(chan_map));

        if (pdu_adv->type == PDU_ADV_TYPE_DIRECT_IND) {
            /* Max. chain is DIRECT_IND * channels + CONNECT_IND */
            ticks_slot = ((RADIO_TICKER_START_PART_US + 176 + 152 + 40) * chan_cnt) - 40 + 352;
            ...
            ticks_slot = (RADIO_TICKER_START_PART_US + 376 + 152 + 176 + 152 + 376) * chan_cnt;
        }
        _radio.advertiser.hdr.ticks_slot = TICKER_US_TO_TICKS(ticks_slot);

        ticks_slot_offset = (_radio.advertiser.hdr.ticks_active_to_start < _radio.advertiser.hdr.ticks_xtal_to_start) ? _radio.advertiser.hdr.ticks_xtal_to_start : _radio.advertiser.hdr.ticks_active_to_start;

        /* High Duty Cycle Directed Advertising if interval is 0. */
        if ((pdu_adv->type == PDU_ADV_TYPE_DIRECT_IND) && !interval) {
            ...
            ret = ticker_start(RADIO_TICKER_INSTANCE_ID_RADIO,
                ...
                TICKER_US_TO_TICKS((u64_t)(1280 * 1000) + RADIO_TICKER_XTAL_OFFSET_US),
                TICKER_NULL_PERIOD, TICKER_NULL_REMAINDER,
                TICKER_NULL_LAZY, TICKER_NULL_SLOT,
    ...
    failure_cleanup:
        ...
        if (conn) {
            mem_release(conn->llcp_terminate.radio_pdu_node_rx.hdr.onion.link, &_radio.link_rx_free);
        ...
        u8_t rpa_gen, u8_t rl_idx)
    {
        ...
        _radio.scanner.ticks_window = TICKER_US_TO_TICKS((u64_t)window * 625);
        _radio.scanner.filter_policy = filter_policy;

        _radio.scanner.hdr.ticks_active_to_start = _radio.ticks_active_to_start;
        _radio.scanner.hdr.ticks_xtal_to_start = TICKER_US_TO_TICKS(RADIO_TICKER_XTAL_OFFSET_US);
        _radio.scanner.hdr.ticks_preempt_to_start = TICKER_US_TO_TICKS(RADIO_TICKER_PREEMPT_PART_MIN_US);
        _radio.scanner.hdr.ticks_slot = _radio.scanner.ticks_window + TICKER_US_TO_TICKS(RADIO_TICKER_START_PART_US);

        ticks_interval = TICKER_US_TO_TICKS((u64_t)interval * 625);
        if (_radio.scanner.hdr.ticks_slot > (ticks_interval - TICKER_US_TO_TICKS(RADIO_TICKER_XTAL_OFFSET_US))) {
            _radio.scanner.hdr.ticks_slot = (ticks_interval - TICKER_US_TO_TICKS(RADIO_TICKER_XTAL_OFFSET_US));
        }

        ticks_slot_offset = (_radio.scanner.hdr.ticks_active_to_start < _radio.scanner.hdr.ticks_xtal_to_start) ? _radio.scanner.hdr.ticks_xtal_to_start : _radio.scanner.hdr.ticks_active_to_start;

        ticks_anchor = ticker_ticks_now_get();

        if ((_radio.scanner.conn) || !IS_ENABLED(CONFIG_BT_CTLR_SCHED_ADVANCED)) {
            us_offset = 0;
        }
#if defined(CONFIG_BT_CTLR_SCHED_ADVANCED)
        else {
        ...
        u16_t latency, u16_t timeout)
    {
        ...
        _radio.scanner.ticks_conn_slot = TICKER_US_TO_TICKS(RADIO_TICKER_START_PART_US + radio_tx_ready_delay_get(0, 0) + 328 + RADIO_TIFS + 328);
        ...
        conn_interval_us = (u32_t)_radio.scanner.conn_interval * 1250;
        conn->supervision_reload = RADIO_CONN_EVENTS((_radio.scanner.conn_timeout * 10 * 1000),
            conn_interval_us);
        conn->supervision_expire = 0;
        conn->procedure_reload = RADIO_CONN_EVENTS((40 * 1000 * 1000), conn_interval_us);
        conn->procedure_expire = 0;
        ...
        u16_t latency, u16_t timeout)
    {
        ...
        if (!cmd) {
            if (conn->common.fex_valid && (conn->llcp_features & BIT(BT_LE_FEAT_BIT_CONN_PARAM_REQ))) {
                cmd++;
            } else if (conn->role) {
                return 1;
            }
        ...
        if (!cmd) {
            ...
            if (cmd) {
                if ((conn->llcp_conn_param.req == conn->llcp_conn_param.ack) || (conn->llcp_conn_param.state != LLCP_CPR_STATE_APP_WAIT)) {
                    return 1;
                }
    ...
    u32_t ll_chm_update(u8_t * chm)
    {
        ...
        _radio.data_chan_count = util_ones_count_get(&_radio.data_chan_map[0],
            sizeof(_radio.data_chan_map));
        ...
        if (node_tx) {
            ...
            if ((conn->enc_rx == 0) && (conn->enc_tx == 0)) {
                ...
                pdu_data_tx->len = offsetof(struct pdu_data_llctrl,
                                       ctrldata)
                    + sizeof(struct pdu_data_llctrl_enc_req);
                pdu_data_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_ENC_REQ;
                enc_req = (void*)&pdu_data_tx->payload.llctrl.ctrldata.enc_req;
                ...
                pdu_data_tx->ll_id = PDU_DATA_LLID_CTRL;
                pdu_data_tx->len = offsetof(struct pdu_data_llctrl, ctrldata);
                pdu_data_tx->payload.llctrl.opcode = PDU_DATA_LLCTRL_TYPE_PAUSE_ENC_REQ;
            } else {
    ...
    u32_t ll_length_req_send(u16_t handle, u16_t tx_octets, u16_t tx_time)
    {
        ...
        conn = connection_get(handle);
        if (!conn || (conn->llcp_req != conn->llcp_ack) || (conn->llcp_length.req != conn->llcp_length.ack)) {
            return 1;
        }
    ...
    u32_t ll_phy_req_send(u16_t handle, u8_t tx, u8_t flags, u8_t rx)
    {
        ...
        conn = connection_get(handle);
        if (!conn || (conn->llcp_req != conn->llcp_ack) || (conn->llcp_phy.req != conn->llcp_phy.ack)) {
            return 1;
        }
    ...
    static u8_t tx_cmplt_get(u16_t * handle, u8_t * first, u8_t last)
    {
        ...
        *handle = _radio.pkt_release[_first].handle;
        do {
            ...
            pdu_data_tx = (struct pdu_data*)node_tx->pdu_data;
            if ((!node_tx) || (node_tx == (struct radio_pdu_node_tx*)1) || ((((u32_t)node_tx & ~(0x00000003)) != 0) && (pdu_data_tx) && (pdu_data_tx->len != 0) && ((pdu_data_tx->ll_id == PDU_DATA_LLID_DATA_START) || (pdu_data_tx->ll_id == PDU_DATA_LLID_DATA_CONTINUE)))) {

                /* data packet, hence count num cmplt */
                _radio.pkt_release[_first].node_tx = (struct radio_pdu_node_tx*)1;

                cmplt++;
            } else {
                /* ctrl packet, hence not num cmplt */
                _radio.pkt_release[_first].node_tx = (struct radio_pdu_node_tx*)2;
            }
    ...
    u8_t radio_rx_get(struct radio_pdu_node_rx * *radio_pdu_node_rx, u16_t * handle)
    {
        ...
        if (_radio.link_rx_head != _radio.link_rx_tail) {
            ...
            cmplt = tx_cmplt_get(handle,
                ...
                _radio_pdu_node_rx->hdr.onion.packet_release_last);
            if (!cmplt) {
                u16_t handle;
                ...
                } while ((cmplt_prev != 0) || (cmplt_prev != cmplt_curr));
    ...
    void radio_rx_dequeue(void)
    {
        ...
        if (radio_pdu_node_rx->hdr.type == NODE_RX_TYPE_CONNECTION) {
            ...
            if ((radio_le_conn_cmplt->status == 0x3c) || radio_le_conn_cmplt->role) {
            ...
            if (conn) {
                struct radio_pdu_node_rx* node_rx = (void*)&conn->llcp_terminate.radio_pdu_node_rx;
            ...
            bm = ((u8_t)_radio.scanner.is_enabled << 1) | _radio.advertiser.is_enabled;
    ...
    u8_t do_radio_rx_fc_set(u16_t handle, u8_t req, u8_t ack)
    {
        ...
        } else if (((req == 0) && (_radio.fc_handle[TRIPLE_BUFFER_SIZE - 1] != handle)) || ((req != 0) && (_radio.fc_handle[req - 1] != handle))) {
            return 1;
        }
    ...
    u8_t radio_rx_fc_set(u16_t handle, u8_t fc)
    {
        if (_radio.fc_ena) {
            ...
            } else if ((_radio.link_rx_head == _radio.link_rx_tail) && (req != ack)) {
                _radio.fc_ack = req;

                if ((_radio.link_rx_head != _radio.link_rx_tail) && (req == _radio.fc_req)) {
                    _radio.fc_ack = ack;
                }
            }
        }
    ...
    u32_t radio_tx_mem_enqueue(u16_t handle, struct radio_pdu_node_tx * node_tx)
    {
        ...
        LL_ASSERT(pdu_data->len <= (_radio.packet_tx_data_size - offsetof(struct radio_pdu_node_tx,
                                                                     pdu_data)
        ...
        if (conn->role && conn->latency_event && !conn->slave.latency_cancel) {
            ...
            ticker_status = ticker_update(RADIO_TICKER_INSTANCE_ID_RADIO,
                RADIO_TICKER_USER_ID_APP,
                RADIO_TICKER_ID_FIRST_CONNECTION + conn->handle, 0, 0, 0, 0, 1, 0,
